...
<script type="text/javascript">
    ...
    function fn_initTable() {
       const columns = [
        ...
        $('#pushList_cardForm').html($('#rcvnCardForm').html());
    ...
    function fn_loadList(pageIndex) {
        ...
        ajaxCall('/msgPushRcvnListAjax.do', param, function(res) {
            if (res.encParams) EPARAM = res.encParams;
            if (res.result > 0) {
    ...
    function fn_rcvnDetail(msgMblSndngId) {
        location.href = '/stdntMsgPushRcvnSelectView.do?encParams=' + EPARAM + '&addParams=' + UiComm.makeEncParams({ msgMblSndngId: msgMblSndngId });
    }

    function fn_deleteSelected() {
        const selectedIds = pushTable.getSelectedData('msgMblSndngId');
        ...
        UiComm.showMessage('<spring:message code="msg.push.msg.confirmDelete"/>', 'confirm').then(function(result) {
            if (!result) return;
            fn_pushDeleteSequential(selectedIds, 0, { failCnt: 0 });
        });
    }

    function fn_pushDeleteSequential(ids, index, state) {
        if (index >= ids.length) {
            if (state.failCnt > 0) {
        ...
        ajaxCall('/msgPushDeleteAjax.do',
            ...
            function(res) {
                ...
                fn_pushDeleteSequential(ids, index + 1, state);
            },
            function() {
                state.failCnt++;
                fn_pushDeleteSequential(ids, index + 1, state);
        ...
        <div id="content" class="content-wrap common">
            <div class="dashboard_sub">
                ...
                <div class="sub-content">
                    ...
                    <!-- 목록 -->
                    <div class="board_top">
                        <h3 class="board-title" id="boardTitle"><spring:message code="msg.push.label.rcvnListTitle" text="푸시 수신 목록"/></h3>
