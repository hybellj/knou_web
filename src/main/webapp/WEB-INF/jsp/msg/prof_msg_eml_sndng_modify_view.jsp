...
<script type="text/javascript">
    ...
    function fn_openTmpltSavePopup() {
        fn_openModal('tmpltSaveModal', '/msgMgrTmpltRegistPopView.do?ttl=' + encodeURIComponent($('#ttl').val() || '') + '&txtCts=' + encodeURIComponent(fn_getEditorContent()));
    }
    ...
    function fn_delExistFile(atflId) {
        const $row = $('#existFile_' + atflId);
        $row.hide();
        const cur = $('#delFileIdStr').val();
        $('#delFileIdStr').val(cur ? (cur + ',' + atflId) : atflId);
    }
    ...
    function fn_save() {
        ...
        UiValidator("msgSndngForm").then(function(result) {
            ...
            const confirmMsg = IS_EDIT_MODE ? '<spring:message code="msg.eml.msg.confirmModify"/>' : '<spring:message code="msg.eml.msg.confirmRegist"/>';
    ...
    function fn_doSave() {
        ...
        const url = IS_EDIT_MODE ? '/msgEmlSndngModifyAjax.do' : '/msgEmlSndngRegistAjax.do';
    ...
    function fn_list() {
        location.href = '/profMsgEmlListView.do?encParams=' + LIST_EPARAM;
    }
</script>
        ...
        <div id="content" class="content-wrap common">
            <div class="dashboard_sub">
                <div class="sub-content">
                    ...
                    <div class="board_top">
                        <h3 class="board-title"><spring:message code="msg.eml.label.sndngModify" text="이메일 발신 수정"/></h3>
                    ...
                    <input type="hidden" name="bfrMsgEmlSndngId" id="bfrMsgEmlSndngId" value="<c:out value='${ed.bfrMsgEmlSndngId}'/>"/>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="editor-box">
                                            ...
                                            <textarea id="htmlCts" name="htmlCts" required="true"><c:choose><c:when test="${not empty ed.htmlCts}"><c:out value="${ed.htmlCts}"/></c:when><c:otherwise><c:out value="${ed.txtCts}"/></c:otherwise></c:choose></textarea>
                                            <script>
                                                let editor = UiEditor({
                                                    targetId: "htmlCts",
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <c:if test="${not empty fileList}">
                                            <ul class="file_list" style="margin-bottom:8px;">
                                                <c:forEach var="atfl" items="${fileList}">
                                                    <li id="existFile_<c:out value='${atfl.atflId}'/>">
                                                        <c:out value="${atfl.atflOriNm}"/>
                                                        <button type="button" class="btn basic small" onclick="fn_delExistFile('<c:out value="${atfl.atflId}"/>')"><spring:message code="msg.common.label.delete" text="삭제"/></button>
                                                    </li>
                                                </c:forEach>
                                            </ul>
                                        </c:if>
                    ...
                    <div class="btns">
                        <button type="button" id="btnSave" class="btn type1" onclick="fn_save()"><spring:message code="msg.common.label.modify" text="수정"/></button>
