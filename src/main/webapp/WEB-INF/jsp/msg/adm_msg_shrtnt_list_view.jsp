...
<script type="text/javascript">
    ...
    function fn_updateBoardTitle() {
        const titleKey = ACTIVE_TAB === 'SNDNG'
            ? '<spring:message code="msg.shrtnt.label.sndngListTitle" text="쪽지 발신 목록"/>'
            : '<spring:message code="msg.shrtnt.label.rcvnListTitle" text="쪽지 수신 목록"/>';
    ...
    function fn_loadList(pageIndex) {
        ...
        const url = ACTIVE_TAB === 'RCVN' ? '/admMsgShrtntRcvnListAjax.do' : '/admMsgShrtntSndngListAjax.do';
    ...
    function fn_rcvnDetail(msgShrtntSndngId) {
        location.href = '/admMsgShrtntRcvnSelectView.do?encParams=' + EPARAM + '&addParams=' + UiComm.makeEncParams({ msgShrtntSndngId: msgShrtntSndngId });
    }

    function fn_sndngDetail(msgId) {
        location.href = '/admMsgShrtntSndngSelectView.do?encParams=' + EPARAM + '&addParams=' + UiComm.makeEncParams({ msgId: msgId });
    }

    function fn_sndngRegist() {
        location.href = '/admMsgShrtntSndngRegistView.do?encParams=' + EPARAM;
    }

    function fn_deleteSelected() {
        const idField = ACTIVE_TAB === 'RCVN' ? 'msgShrtntSndngId' : 'msgId';
        const selectedIds = shrtntTable.getSelectedData(idField);
        ...
        UiComm.showMessage('<spring:message code="msg.shrtnt.msg.confirmDelete"/>', 'confirm').then(function(result) {
            if (!result) return;
            fn_shrtntDeleteSequential(selectedIds, idField, 0, { failCnt: 0 });
        });
    }

    function fn_shrtntDeleteSequential(ids, idField, index, state) {
        if (index >= ids.length) {
            if (state.failCnt > 0) {
        ...
        ajaxCall('/admMsgShrtntDeleteAjax.do',
            { encParams: EPARAM, addParams: UiComm.makeEncParams(delData) },
            function(res) {
                if (res.encParams) EPARAM = res.encParams;
                if (res.result !== 1) state.failCnt++;
                fn_shrtntDeleteSequential(ids, idField, index + 1, state);
            },
            function() {
                state.failCnt++;
                fn_shrtntDeleteSequential(ids, idField, index + 1, state);
        ...
        <div id="content" class="content-wrap common">
            <div class="admin_sub">
                ...
                <div class="sub-content">
                    ...
                    <!-- 목록 -->
                    <div class="board_top">
                        <h3 class="board-title" id="boardTitle"><c:choose><c:when test="${vo.listType eq 'SNDNG'}"><spring:message code="msg.shrtnt.label.sndngListTitle" text="쪽지 발신 목록"/></c:when><c:otherwise><spring:message code="msg.shrtnt.label.rcvnListTitle" text="쪽지 수신 목록"/></c:otherwise></c:choose></h3>
