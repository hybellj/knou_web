...
<script type="text/javascript">
    ...
    const TMPLT_PREFIX = IS_ADMIN ? '/admMsgTmplt' : '/msgTmplt';
    let tmpltList = [];
    ...
    function fn_initContent() {
        ...
        const txtCts = params.get('txtCts') || '';
        $('#saveTtl').val(ttl);
        $('#saveCts').val(txtCts);
    }

    function fn_loadList() {
        const data = { pageIndex: 1, listScale: 100, msgCtsGbncd: 'INDV_MSG' };
        ajaxCall(TMPLT_PREFIX + 'ListAjax.do', { encParams: EPARAM, addParams: UiComm.makeEncParams(data) }, function(res) {
    ...
    function fn_save() {
        const ttl = $('#saveTtl').val().trim();
        if (!ttl) {
            UiComm.showMessage('<spring:message code="msg.modal.requiredTtl"/>', 'warning');
            $('#saveTtl').focus();
            return;
        }
        ...
        ajaxCall(TMPLT_PREFIX + 'RegistAjax.do', param, function(res) {
            if (res.encParams) EPARAM = res.encParams;
            if (res.result > 0) {
...
<div id="loading_page" style="display:none;"></div>
    ...
    <div class="table-wrap">
        <table class="table-type5">
            ...
            <tbody>
                <tr>
                    <th><label for="saveTtl" class="req"><spring:message code="msg.modal.ttl" text="제목"/></label></th>
                    <td>
                        <div class="form-row">
                            <input class="form-control width-100per" type="text" id="saveTtl" placeholder="<spring:message code="msg.modal.ttlPlaceholder" text="제목입력"/>">
                        </div>
                    </td>
                </tr>
                <tr>
                    <th><label for="saveCts"><spring:message code="msg.modal.cts" text="내용"/></label></th>
                    <td>
    ...
    <div class="modal_btns mt10">
        <button type="button" class="btn type1" onclick="fn_save()"><spring:message code="msg.modal.tmpltSaveBtn" text="저장하기"/></button>
    </div>

    <!-- 기존 템플릿 카드 목록 -->
    <div class="board_top">
        <h4 class="sub-title"><spring:message code="msg.modal.indvMsg" text="개인 문구"/></h4>
    </div>
    ...
    <div class="modal_btns">
        <button type="button" class="btn type2" onclick="parent.tmpltSaveDlg.close()"><spring:message code="msg.modal.close" text="닫기"/></button>
    </div>
