...
<script type="text/javascript">
    ...
    function fn_loadSmstrList(onDone) {
        ...
        ajaxCall('/msgMgrSmstrListAjax.do', { encParams: EPARAM, addParams: UiComm.makeEncParams({ sbjctYr: yr }) }, function(res) {
    ...
    function fn_loadSbjctList() {
        ...
        ajaxCall('/msgMgrSbjctListAjax.do', { encParams: EPARAM, addParams: UiComm.makeEncParams({ orgId: orgId }) }, function(res) {
            if (res.encParams) EPARAM = res.encParams;
            let html = allHtml;
            let foundOrig = false;
    ...
    function fn_openRcvrPopup() {
        fn_openModal('rcvrModal', '/msgMgrRcvrPopView.do');
    }
    ...
    function fn_openTmpltSavePopup() {
        fn_openModal('tmpltSaveModal', '/msgMgrTmpltRegistPopView.do?ttl=' + encodeURIComponent($('#ttl').val() || '') + '&txtCts=' + encodeURIComponent($('#txtCts').val() || ''));
    }
    ...
    function fn_save() {
        ...
        UiValidator("msgSndngForm").then(function(result) {
            ...
            if (RCVR_LIST.length === 0) {
                UiComm.showMessage("<spring:message code='msg.shrtnt.msg.requiredRcvr'/>", "warning");
                return;
            }
            ...
            if (!isRsrv) {
                ...
                const confirmMsg = IS_EDIT_MODE ? '<spring:message code="msg.shrtnt.msg.confirmModify"/>' : '<spring:message code="msg.shrtnt.msg.confirmRegist"/>';
    ...
    function fn_checkRcptnRjct() {
        const data = { rcvrListJson: JSON.stringify(RCVR_LIST) };
        const param = { encParams: EPARAM, addParams: UiComm.makeEncParams(data) };

        ajaxCall('/msgShrtntRcptnRjctCntSelectAjax.do', param, function(res) {
            if (res.encParams) EPARAM = res.encParams;
            if (res.result > 0) {
                const info = res.returnVO;
                const totalCnt = info.totalCnt || 0;
                const allowedCnt = info.allowedCnt || 0;
                const rejectedCnt = info.rejectedCnt || 0;

                if (allowedCnt === 0) {
                    UiComm.showMessage('<spring:message code="msg.shrtnt.msg.allRcptnRjct"/>', 'error');
                    return;
                }

                let confirmMsg;
                if (rejectedCnt > 0) {
                    confirmMsg = '<spring:message code="msg.shrtnt.msg.rcptnRjctConfirm"/>'.replace('{0}', rejectedCnt).replace('{1}', totalCnt).replace('{2}', allowedCnt);
                } else {
                    confirmMsg = IS_EDIT_MODE ? '<spring:message code="msg.shrtnt.msg.confirmModify"/>' : '<spring:message code="msg.shrtnt.msg.confirmRegist"/>';
                }
    ...
    function fn_doSave() {
        ...
        const url = IS_EDIT_MODE ? '/msgShrtntSndngModifyAjax.do' : '/msgShrtntSndngRegistAjax.do';
        ...
        <div id="content" class="content-wrap common">
            <div class="dashboard_sub">
                <div class="sub-content">
                    ...
                    <div class="board_top">
                        <h3 class="board-title"><spring:message code="msg.common.label.sndngRegist" text="쪽지 발신하기"/></h3>
                    ...
                    <input type="hidden" name="upMsgShrtntSndngId" id="upMsgShrtntSndngId" value="<c:out value='${ed.upMsgShrtntSndngId}'/>"/>
                        ...
                        <table class="table-type5">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="form-row">
                                            <input type="text" id="ttl" name="ttl" class="form-control width-100per" placeholder="<spring:message code="msg.common.label.ttl"/>" inputmask="length" maxLen="200" value="<c:out value='${ed.ttl}'/>" required="true">
                                        </div>
                                    </td>
                                </tr>
                                <tr>
                                ...
                                <tr>
                                    <th><label><spring:message code="msg.shrtnt.label.atfl" text="첨부파일"/></label></th>
                                    <td>
                    ...
                    <!-- 받는 사람 -->
                    <div class="board_top">
                        <h4 class="sub-title"><spring:message code="msg.shrtnt.label.rcvrList" text="받는 사람"/></h4>
                        <div class="right-area">
