...
<script type="text/javascript">
    ...
    function fn_validate() {
        if (!$('#popupNtcTtl').val().trim()) {
            UiComm.showMessage('<spring:message code="msg.popupNtc.msg.ttlRequired"/>', 'warning');
            $('#popupNtcTtl').focus();
            return false;
        }
        if (!$('#popupWinWdthsz').val() || !$('#popupWinHght').val()) {
            UiComm.showMessage('<spring:message code="msg.popupNtc.msg.sizeRequired"/>', 'warning');
            return false;
        }
        if (!$('#popupWinXcrd').val() || !$('#popupWinYcrd').val()) {
            UiComm.showMessage('<spring:message code="msg.popupNtc.msg.pstnRequired"/>', 'warning');
            return false;
        }
        if (!$('input[name=popupPnttimeGbncd]:checked').val()) {
            UiComm.showMessage('<spring:message code="msg.popupNtc.msg.pnttimeRequired"/>', 'warning');
            return false;
        }
        if (!$('#chkTrgtAll').is(':checked') && $('input[name=trgtCd]:checked').length === 0) {
            UiComm.showMessage('<spring:message code="msg.popupNtc.msg.trgtRequired"/>', 'warning');
            return false;
        }
        if (!$('#popupNtcSdttm').val() || !$('#popupNtcEdttm').val()) {
            UiComm.showMessage('<spring:message code="msg.popupNtc.msg.periodRequired"/>', 'warning');
            return false;
        }
    ...
    function fn_save() {
        ...
        let url = '/admPopupNtcRegistAjax.do';
        let successMsg = '<spring:message code="msg.popupNtc.msg.saveSuccess"/>';

        if (mode === 'modify') {
            extData.popupNtcId = '${detailVO.popupNtcId}';
            url = '/admPopupNtcModifyAjax.do';
        ...
        ajaxCall(url, param, function(res) {
            ...
            if (res.result > 0) {
                ...
                location.href = '/admPopupNtcListView.do?encParams=' + EPARAM;
            } else {
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <div class="form-row">
                                                    <div class="date_area">
                                                        <input type="text" id="popupNtcSdttm" name="popupNtcSdttm" placeholder="<spring:message code='msg.popupNtc.label.startDate' text='시작일'/>" class="datepicker" toDate="popupNtcEdttm" timeId="sdttmTime">
                                                        <input type="text" id="sdttmTime" name="sdttmTime" placeholder="<spring:message code='msg.popupNtc.label.startTime' text='시작시간'/>" class="timepicker" dateId="popupNtcSdttm">
                                                        <span class="txt-sort">~</span>
                                                        <input type="text" id="popupNtcEdttm" name="popupNtcEdttm" placeholder="<spring:message code='msg.popupNtc.label.endDate' text='종료일'/>" class="datepicker" fromDate="popupNtcSdttm" timeId="edttmTime">
                                                        <input type="text" id="edttmTime" name="edttmTime" placeholder="<spring:message code='msg.popupNtc.label.endTime' text='종료시간'/>" class="timepicker" dateId="popupNtcEdttm">
                                                    </div>
                                                </div>
                                            </td>
                                        </tr>
