...
<script type="text/javascript">
    ...
    const AUTO_ALIM_MSG_ID = '<c:out value="${detailVo.autoAlimMsgId}" />';
    ...
    function fn_initForm() {
        $('#orgId').val('<c:out value="${detailVo.orgId}" />');
        $('#autoAlimnm').val('<c:out value="${detailVo.autoAlimnm}" />');
        $('#autoAlimMsgCts').val('<c:out value="${detailVo.autoAlimMsgCts}" />');
        $('#sndngCndtnCts').val('<c:out value="${detailVo.sndngCndtnCts}" />');

        <c:if test="${detailVo.autoAlimTrgtTycds != null}">
            <c:forEach var="tycd" items="${detailVo.autoAlimTrgtTycds}">
                $('input[name=autoAlimTrgtTycds][value=<c:out value="${tycd}"/>]').prop('checked', true);
            </c:forEach>
        </c:if>
    }
    ...
    function fn_save() {
        ...
        let extData = {
            autoAlimMsgId: AUTO_ALIM_MSG_ID,
            orgId: $('#orgId').val(),
        ...
        ajaxCall('/admAutoAlimMsgModifyAjax.do', param, function(res) {
                                                ...
                                                <div class="form-row">
                                                    <select class="form-select" id="orgId">
                                                        ...
                                                        <c:forEach var="org" items="${orgList}">
                                                            <option value="${org.orgId}" <c:if test="${org.orgId eq detailVo.orgId}">selected</c:if>><c:out value="${org.orgnm}"/></option>
                                                        </c:forEach>
                                                    </select>
                                                </div>
                                            </td>
