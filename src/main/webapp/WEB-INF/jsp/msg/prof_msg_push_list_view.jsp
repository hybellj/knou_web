...
<script type="text/javascript">
    ...
    function fn_updateBoardTitle() {
        const titleKey = ACTIVE_TAB === 'SNDNG'
            ? '<spring:message code="msg.push.label.sndngListTitle" text="푸시 발신 목록"/>'
            : '<spring:message code="msg.push.label.rcvnListTitle" text="푸시 수신 목록"/>';
    ...
    function fn_initTable() {
        if (pushTable) {
            pushTable.destroy();
            $('#pushList').parent().css({'width': '', 'max-width': ''});
        ...
        if (ACTIVE_TAB === 'RCVN') {
            ...
            columns = [
                ...
                {title:"<spring:message code='msg.common.col.sndngnm' text='발신자'/>",     field:"sndngnm",       headerHozAlign:"center", hozAlign:"center", width:90,  minWidth:50},
                {title:"<spring:message code='msg.common.col.ttl' text='제목'/>",           field:"ttl",           headerHozAlign:"center", hozAlign:"left",   width:0,   minWidth:150, formatter:"html"},
                {title:"<spring:message code='msg.push.col.rcvrCnt' text='수신자'/>",     field:"rcvrCnt",       headerHozAlign:"center", hozAlign:"center", width:55,  minWidth:40},
                ...
                {title:"<spring:message code='msg.push.col.sndngYn' text='발신'/>",       field:"sndngYn",       headerHozAlign:"center", hozAlign:"center", width:45,  minWidth:35},
        ...
        $('#pushList_cardForm').html($('#' + cardFormId).html());

        pushTable = UiTable("pushList", {
            lang: "ko",
            selectRow: "checkbox",
    ...
    function fn_loadList(pageIndex) {
        ...
        const url = ACTIVE_TAB === 'RCVN' ? '/msgPushRcvnListAjax.do' : '/msgPushSndngListAjax.do';
        ...
        ajaxCall(url, param, function(res) {
            ...
            if (res.result > 0) {
                ...
                pushTable.clearData();
                pushTable.replaceData(dataList);
                pushTable.setPageInfo(res.pageInfo);
            } else {
    ...
    function fn_createRcvnListData(list, pageInfo) {
        ...
        list.forEach(function(v, i) {
            ...
            const ttlHtml = '<a href="javascript:fn_rcvnDetail(\'' + v.msgMblSndngId + '\')" class="title link">' + UiComm.escapeHtml(v.sndngTtl || '') + '</a>';
    ...
    function fn_rcvnDetail(msgMblSndngId) {
        location.href = '/profMsgPushRcvnSelectView.do?encParams=' + EPARAM + '&addParams=' + UiComm.makeEncParams({ msgMblSndngId: msgMblSndngId });
    }

    function fn_sndngDetail(msgId) {
        location.href = '/profMsgPushSndngSelectView.do?encParams=' + EPARAM + '&addParams=' + UiComm.makeEncParams({ msgId: msgId });
    }

    function fn_sndngRegist() {
        location.href = '/profMsgPushSndngRegistView.do?encParams=' + EPARAM;
    }

    function fn_deleteSelected() {
        const idField = ACTIVE_TAB === 'RCVN' ? 'msgMblSndngId' : 'msgId';
        const selectedIds = pushTable.getSelectedData(idField);
        if (!selectedIds || selectedIds.length === 0) {
            UiComm.showMessage('<spring:message code="common.item.select.msg"/>', 'warning');
            return;
        }

        UiComm.showMessage('<spring:message code="msg.push.msg.confirmDelete"/>', 'confirm').then(function(result) {
            if (!result) return;
            fn_pushDeleteSequential(selectedIds, idField, 0, { failCnt: 0 });
        });
    }

    function fn_pushDeleteSequential(ids, idField, index, state) {
        if (index >= ids.length) {
            if (state.failCnt > 0) {
    ...
    function fn_doRsrvCncl() {
        ...
        ajaxCall('/msgPushRsrvCnclModifyAjax.do', param, function(res) {
            if (res.encParams) EPARAM = res.encParams;
            if (res.result > 0) {
        ...
        <div id="content" class="content-wrap common">
            <div class="dashboard_sub">
                ...
                <div class="sub-content">
                    ...
                    <!-- 목록 -->
                    <div class="board_top">
                        <h3 class="board-title" id="boardTitle"><c:choose><c:when test="${vo.listType eq 'SNDNG'}"><spring:message code="msg.push.label.sndngListTitle" text="푸시 발신 목록"/></c:when><c:otherwise><spring:message code="msg.push.label.rcvnListTitle" text="푸시 수신 목록"/></c:otherwise></c:choose></h3>
                    ...
                    <!-- 푸시 알림 카드 폼 -->
                    <div id="pushList_cardForm" style="display:none"></div>
                    ...
                    <div id="sndngCardForm" style="display:none">
                        ...
                        <div class="card-body">
                            ...
                            <div class="etc">
                                <p><label class="label-title"><spring:message code='msg.push.col.rcvrCnt' text='수신자'/></label><strong>#[rcvrCnt]</strong></p>
                                <p><label class="label-title"><spring:message code='msg.push.col.sndngYn' text='발신'/></label><strong>#[sndngYn]</strong></p>
                            </div>
                        </div>
                    </div>
        ...
        <div class="modal-body">
            <div class="msg-box">
                <p class="txt">
                    ...
                    <span><spring:message code="msg.push.msg.rsrvCnclWarn"/></span>
                </p>
            </div>
            ...
            <div class="table_list">
                ...
                <ul class="list">
                    <li class="head"><label><spring:message code="msg.push.label.rcvrCnt" text="수신자"/></label></li>
