...
<script type="text/javascript">
    ...
    function fn_save() {
        let isRsrv = $('#rsrvYnChk').is(':checked');
        if (isRsrv) {
        ...
        UiValidator("msgSndngForm").then(function(result) {
            ...
            if (!isRsrv) {
                ...
                let confirmMsg = IS_EDIT_MODE ? '<spring:message code="msg.shrtnt.msg.confirmModify"/>' : '<spring:message code="msg.shrtnt.msg.confirmRegist"/>';
    ...
    function fn_checkRcptnRjct() {
        let data = { rcvrListJson: JSON.stringify(RCVR_LIST) };
        let param = { encParams: EPARAM, addParams: UiComm.makeEncParams(data) };

        ajaxCall('/admMsgShrtntRcptnRjctCntSelectAjax.do', param, function(res) {
            if (res.encParams) EPARAM = res.encParams;
            if (res.result > 0) {
                let info = res.returnVO;
                let totalCnt = info.totalCnt || 0;
                let allowedCnt = info.allowedCnt || 0;
                let rejectedCnt = info.rejectedCnt || 0;
    ...
    function fn_doSave() {
        ...
        let url = IS_EDIT_MODE ? '/admMsgShrtntSndngModifyAjax.do' : '/admMsgShrtntSndngRegistAjax.do';
        let successMsg = IS_EDIT_MODE ? '<spring:message code="msg.common.msg.modifySuccess"/>' : '<spring:message code="msg.common.msg.registSuccess"/>';
        let data = $("#msgSndngForm").serialize();
    ...
    function fn_list() {
        location.href = '/admMsgShrtntListView.do?encParams=' + LIST_EPARAM;
    }
</script>
