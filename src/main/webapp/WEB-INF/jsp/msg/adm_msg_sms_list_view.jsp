...
<script type="text/javascript">
    ...
    function fn_updateBoardTitle() {
        const titleKey = ACTIVE_TAB === 'SNDNG'
            ? '<spring:message code="msg.sms.label.sndngListTitle" text="문자 발신 목록"/>'
            : '<spring:message code="msg.sms.label.rcvnListTitle" text="문자 수신 목록"/>';
    ...
    function fn_initTable() {
        if (smsTable) {
            smsTable.destroy();
            $('#smsList').parent().css({'width': '', 'max-width': ''});
        }
        ...
        if (ACTIVE_TAB === 'RCVN') {
            columns = [
                ...
                {title:"<spring:message code='msg.sms.col.chnl' text='채널'/>",          field:"chnl",        headerHozAlign:"center", hozAlign:"center", width:55,  minWidth:45},
            ...
            columns = [
                ...
                {title:"<spring:message code='msg.common.col.sndngnm' text='발신자'/>",     field:"sndngnm",       headerHozAlign:"center", hozAlign:"center", width:90,  minWidth:50},
                {title:"<spring:message code='msg.common.col.cts' text='내용'/>",           field:"cts",           headerHozAlign:"center", hozAlign:"left",   width:0,   minWidth:150, formatter:"html"},
                {title:"<spring:message code='msg.sms.col.rcvrCnt' text='수신자'/>",     field:"rcvrCnt",       headerHozAlign:"center", hozAlign:"center", width:55,  minWidth:40},
                ...
                {title:"<spring:message code='msg.sms.col.sndngYn' text='발신'/>",       field:"sndngYn",       headerHozAlign:"center", hozAlign:"center", width:45,  minWidth:35},
    ...
    function fn_loadList(pageIndex) {
        ...
        const url = ACTIVE_TAB === 'RCVN' ? '/admMsgSmsRcvnListAjax.do' : '/admMsgSmsSndngListAjax.do';
        ...
        ajaxCall(url, param, function(res) {
            ...
            if (res.result > 0) {
                ...
                smsTable.clearData();
                smsTable.replaceData(dataList);
                smsTable.setPageInfo(res.pageInfo);
            } else {
    ...
    function fn_rcvnDetail(msgMblSndngId) {
        location.href = '/admMsgSmsRcvnSelectView.do?encParams=' + EPARAM + '&addParams=' + UiComm.makeEncParams({ msgMblSndngId: msgMblSndngId });
    }

    function fn_sndngDetail(msgId) {
        location.href = '/admMsgSmsSndngSelectView.do?encParams=' + EPARAM + '&addParams=' + UiComm.makeEncParams({ msgId: msgId });
    }

    function fn_sndngRegist() {
        location.href = '/admMsgSmsSndngRegistView.do?encParams=' + EPARAM;
    }

    function fn_deleteSelected() {
        const idField = ACTIVE_TAB === 'RCVN' ? 'msgMblSndngId' : 'msgId';
        const selectedIds = smsTable.getSelectedData(idField);
        if (!selectedIds || selectedIds.length === 0) {
            UiComm.showMessage('<spring:message code="common.item.select.msg"/>', 'warning');
            return;
        }

        UiComm.showMessage('<spring:message code="msg.sms.msg.confirmDelete"/>', 'confirm').then(function(result) {
            if (!result) return;
            fn_smsDeleteSequential(selectedIds, idField, 0, { failCnt: 0 });
        });
    }

    function fn_smsDeleteSequential(ids, idField, index, state) {
        if (index >= ids.length) {
            if (state.failCnt > 0) {
    ...
    function fn_doRsrvCncl() {
        ...
        ajaxCall('/admMsgSmsRsrvCnclModifyAjax.do', param, function(res) {
            if (res.encParams) EPARAM = res.encParams;
            if (res.result > 0) {
        ...
        <div id="content" class="content-wrap common">
            <div class="admin_sub">
                ...
                <div class="sub-content">
                    ...
                    <!-- 목록 -->
                    <div class="board_top">
                        <h3 class="board-title" id="boardTitle"><c:choose><c:when test="${vo.listType eq 'SNDNG'}"><spring:message code="msg.sms.label.sndngListTitle" text="문자 발신 목록"/></c:when><c:otherwise><spring:message code="msg.sms.label.rcvnListTitle" text="문자 수신 목록"/></c:otherwise></c:choose></h3>
        ...
        <div class="modal-body">
            <div class="msg-box">
                <p class="txt">
                    ...
                    <span><spring:message code="msg.sms.msg.rsrvCnclWarn"/></span>
                </p>
            </div>
            ...
            <div class="table_list">
                ...
                <ul class="list">
                    <li class="head"><label><spring:message code="msg.sms.label.rcvrCnt" text="수신자"/></label></li>
