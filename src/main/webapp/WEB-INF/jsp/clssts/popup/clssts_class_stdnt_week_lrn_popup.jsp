    ...
    <div class="sub-box">
        ...
        <div class="user-wrap mb10">
            ...
            <div class="table_list">
                <ul class="list">
                    <li class="head"><label><spring:message code="cls.label.org"/></label></li><%-- 기관 --%><li id="infoOrg">-</li></ul>
                <ul class="list">
                    <li class="head"><label><spring:message code="common.name"/></label></li><%-- 이름 --%><li id="infoNm">-</li></ul>
                <ul class="list">
                    <li class="head"><label><spring:message code="cls.label.student.no"/></label></li><%-- 학번 --%><li id="infoStdntNo">-</li></ul>
                <ul class="list">
                    <li class="head"><label><spring:message code="common.id"/></label></li><%-- 아이디 --%><li id="infoUserId">-</li></ul>
                <ul class="list">
                    <li class="head"><label><spring:message code="cls.label.mobile.number"/></label></li><%-- 휴대폰번호 --%><li id="infoMobile">-</li></ul>
                <ul class="list">
                    <li class="head"><label><spring:message code="cls.label.user.email"/></label></li><%-- 사용 이메일 --%><li id="infoEmail">-</li></ul>
            </div>
        </div>
    </div>
...
<script type="text/javascript">
    ...
    function loadWkDetail() {
        ...
        ajaxCall(CTX + "/clssts/selectClsStsClassStdntWkLrnSummary.do", {sbjctId: sbjctId, userId: userId, wkNo: wkNo}, function (res) {
                if (!res || res.result !== 1) {
    ...
    function renderWkSummary(d, wkNo) {
        ...
        var stsCls = sts === "ATND" ? "state_ok"
            : sts === "LATE" ? "state_late"
                : sts === "ABSNT" ? "state_no"
                : "state_none";
        var stsLbl = sts === "ATND" ? '<spring:message code="cls.label.attendance"/><%-- 출석 --%>'
            : sts === "LATE" ? '<spring:message code="cls.label.late"/><%-- 지각 --%>'
                : sts === "ABSNT" ? '<spring:message code="cls.label.absence"/><%-- 결석 --%>'
        ...
        var inPrd  = (d.inPrdLrnMin || 0) + '<spring:message code="date.minute"/><%-- 분 --%> '
            + (d.inPrdLrnSec || 0) + '<spring:message code="date.second"/><%-- 초 --%>';
        var aftPrd = (d.aftPrdLrnMin || 0) + '<spring:message code="date.minute"/><%-- 분 --%> '
            + (d.aftPrdLrnSec || 0) + '<spring:message code="date.second"/><%-- 초 --%>';
        $("#lrnTimeSpan").text(inPrd + " ( " + '<spring:message code="cls.label.after.period"/><%-- 기간 후 --%>' + " : " + aftPrd + " )");
    ...
    function renderChsiList(list) {
        ...
        list.forEach(function (chsi, idx) {
            ...
            var stscrscls = lrnSts === "학습완료" ? "state_ok" : lrnSts === "학습중" ? "state_late" : "state_no";

            var $li = $('<li class="' + (isOpen ? "active" : "") + '" id="' + liId + '"></li>');

            var $titleWrap = $(
                '<div class="title-wrap">'
                + '<div class="chasi_tit">[ ' + UiComm.escapeHtml(String(chsi.chsiNo || "")) + '차시 ] '
            ...
            var $lbox = $(
                ...
                + '<p class="labels"><label class="label s_basic">'
                + UiComm.escapeHtml(chsi.cntntsTypeNm || "")
                + '</label></p>'
                + '<strong>' + UiComm.escapeHtml(chsi.cntntsTitle || "") + '</strong>'
                + '</div>'
                + '<div class="btn_right">'
                + '<label class="state ' + stscrscls + '">' + UiComm.escapeHtml(lrnSts || "-") + '</label>'
                + '</div>'
    ...
    function loadChsiLog(cntntsId, idx) {
        ...
        ajaxCall(CTX + "/clssts/selectClsStsClassStdntLrnLog.do", {
                sbjctId: sbjctId,
                userId: userId,
    ...
    function doAtndProcess() {
        ...
        UiComm.showMessage('<spring:message code="cls.confirm.attend.process.week"/><%-- {0}주차 출석 처리하시겠습니까? --%>'.replace('{0}', wkNo), "confirm")
            .then(function (result) {
                if (!result) return;
                ajaxCall(CTX + "/clssts/updateClsStsClassAtndlcProcess.do", {
                        sbjctId: sbjctId,
                        userId: userId,
                        wkNo: wkNo,
    ...
    function doAtndCancel() {
        ...
        UiComm.showMessage('<spring:message code="cls.confirm.attend.cancel.week"/><%-- {0}주차 출석 처리를 취소하시겠습니까? --%>'.replace('{0}', wkNo), "confirm")
            .then(function (result) {
                if (!result) return;
                ajaxCall(CTX + "/clssts/updateClsStsClassAtndlcCancel.do", {
                        sbjctId: sbjctId,
                        userId: userId,
                        wkNo: wkNo,
                        lctrWknoSchdlId: currentWkSchdlId
                    ...
                    function () {
                        UiComm.showMessage('<spring:message code="fail.common.msg"/><%-- 에러가 발생하였습니다 --%>', "error");
                    },
                    true
                );
            });
