...
<script type="text/javascript">
    ...
    function loadElemSbmsnList() {
        ajaxCall(CTX + "/clssts/selectClsStsClassStdntElemSbmsnList.do", {sbjctId: sbjctId, userId: userId, elemType: elemType}, function (res) {
    ...
    function loadSbmsnLog(cntntsId, idx, colCount) {
        ajaxCall(CTX + "/clssts/selectClsStsClassStdntElemSbmsnLog.do", {sbjctId: sbjctId, cntntsId: cntntsId, userId: userId, elemType: elemType}, function (res) {
                ...
                list.forEach(function (r, i) {
                    var row = '<tr>'
                        + '<td class="t_center" data-th="' + '<spring:message code="common.number.no"/>' + '">' + (i + 1) + '</td>'; <%-- 번호 --%>
                    ...
                    if (elemType === 'ASMT') {
                        ...
                        var downloadHtml = encDownParam
                            ? '<button type="button" class="btn basic small fileDownBtn" data-enc-down-param="' + UiComm.escapeHtml(encDownParam) + '"><spring:message code="button.download" /></button>' <%-- 다운로드 --%>
                            : '-';
                        row += '<td class="t_center" data-th="' + '<spring:message code="cls.label.submit.datetime" />' + '">' + UiComm.escapeHtml(r.sbmsnDttm || '') + '</td>' <%-- 제출일시 --%>
                            + '<td class="t_left" data-th="' + '<spring:message code="cls.label.attachment" />' + '">' + UiComm.escapeHtml(r.fileNm || '') + '</td>' <%-- 첨부파일 --%>
                            + '<td class="t_center" data-th="' + '<spring:message code="common.label.list.size" />' + '">' + UiComm.escapeHtml(r.fileSzText || '') + '</td>' <%-- 크기 --%>
                        ...
                        row += '<td class="t_center" data-th="' + participateDatetimeLabel + '">' + UiComm.escapeHtml(r.sbmsnDttm || '') + '</td>'
                            + '<td class="t_left" data-th="' + '<spring:message code="cls.label.content" />' + '">' + forumPostLabel + ' ' <%-- 내용 --%>
                            + UiComm.escapeHtml(String(r.postCnt || '0')) + ', ' + forumCommentLabel + ' ' + UiComm.escapeHtml(String(r.commentCnt || '0')) + '</td>';
                    } else {
                        row += '<td class="t_center" data-th="' + participateDatetimeLabel + '">' + UiComm.escapeHtml(r.sbmsnDttm || '') + '</td>'
                            + '<td class="t_left" data-th="' + '<spring:message code="cls.label.content" />' + '">' + UiComm.escapeHtml(r.contents || '') + '</td>'; <%-- 내용 --%>
                    }
                    row += '</tr>';
                    $tbody.append(row);
                });
