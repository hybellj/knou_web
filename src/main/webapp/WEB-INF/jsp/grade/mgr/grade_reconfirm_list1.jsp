	...
	<script type="text/javascript">
		...
		$(function(){
			...
			$("#btnObjtAdd").on("click", function(){
				if($("#selCrsCreCd").val() == "") {
					/* 과목이 없습니다. */
					alert('<spring:message code="std.alert.not.crscre" />');
					return false;
				}
				...
				if(data.returnList.length > 0){
					$.each(data.returnList, function(i, o){
			            ...
			            //if(gfn_isNull(o.procCd)){
			            	html += "        <a href=\"javascript:onObjtProcPop('" + o.objtUserId + "','" + o.crsCreCd + "','" + o.uniCd + "');\" class='ui basic small button'><spring:message code='exam.label.process.process' /></a>  ";
			            //} else {
			            //	html += "-";
			            //}
			            html += "    </td>";
		...
		//처리하기 팝업
		function onObjtProcPop(objtUserId, crsCreCd, uniCd){
    ...
    <div id="wrap" class="pusher">
            ...
            <div class="content" >
            	<div class="ui form">
			    	...
			    	<div class="option-content mb10">
                   	...
                   	<table class="tBasic type2" data-sorting="true" data-paging="false" data-empty="<spring:message code='common.nodata.msg' />"><!-- 등록된 내용이 없습니다. -->
					   	<thead>
					   		<tr>
		...
		<!-- 사유보기 팝업 -->
    		...
    		<form class="ui form" id="modalObjtProcForm" name="modalObjtProcForm" method="POST" action="">
		    	...
		    	<input type="hidden" name="crsCreCd"/>
			    <div class="modal fade" id="modalObjtProc" tabindex="-1" role="dialog" aria-labelledby="모달영역" aria-hidden="false">
			        <div class="modal-dialog modal-lg" role="document">
			            <div class="modal-content">
			                <div class="modal-header">
			                    <button type="button" class="close" data-dismiss="modal" aria-label="<spring:message code="sys.button.close" />">
