	...
	<script type="text/javascript">
		...
		function exrcsQstnRegist() {
			...
			validator.then(function(result) {
				if (result) {
					let url = "/quiz/admExrcsQstnRegistAjax.do";
					if(${not empty vo.exrcsSddnQstnBscId}) {
						url = "/quiz/admExrcsQstnModifyAjax.do";
					}

					$("#sbjctIds").val($("#sbjctId").val());
					ajaxCall(url, $("#exrcsQstnRegistFrm").serialize(), function (data) {
		                if (data.result > 0) {
							...
							.then(function(result) {
								exrcsSddnViewMv(data.data.exrcsSddnQstnBscId, "ADMEXRCSMODIFY", "EXRCS_QSTN", ($("#sbjctId").val() || "${vo.sbjctId}"));	// 관리자 연습문제 수정 화면
							});
		                } else {
		 ...
		 */
		 function qstnListSelect() {
			...
			items.forEach(item => {
		 	...
		 	const url  = "/quiz/admQuizQstnListAjax.do";
		 	const data = {
		...
		function qstnRegist(parentId, formId) {
			UiValidator(formId).then(function(result) {
				if (result) {
					...
					const url = "/quiz/admExrcsQstnBulkQstnRegistAjax.do";
		...
		function qstnModify(qstnSeqno) {
			...
			UiValidator(formId).then(function(result) {
				if (result) {
					...
					const url = "/quiz/admExrcsQstnBulkQstnModifyAjax.do";
		...
		function qstnSeqnoChange(obj) {
			...
			if(qstnSeqno != newqstnSeqno) {
				const url  = "/quiz/admExrcsQstnBulkQstnSeqnoModifyAjax.do";
				const data = {
					exrcsSddnQstnBscId	: "${vo.exrcsSddnQstnBscId}",
		...
		function qstnDelete(exrcsSddnQstnBscId, qstnSeqno) {
			...
			const url  = "/quiz/admExrcsQstnBulkQstnDeleteAjax.do";
			const data = {
		...
		function qstnCopyPopup() {
			...
			dialog = UiDialog("dialog1", {
				...
				width	: 800,
				height	: 650,
				url		: "/quiz/admExrcsQstnCopyPopup.do?encParams="+EPARAM+"&addParams="+UiComm.makeEncParams(extData)
			});
		}
		...
		function qstnExcelUploadPopup() {
			...
			dialog = UiDialog("dialog1", {
				...
				width		: 600,
				height		: 500,
				url			: "/quiz/admQstnExcelUploadPopup.do?"+data,
				autoresize	: true
			});
		}
		...
		function qstnsCmptnModify(type) {
			if(exrcsOption.canQstnEdit("submit")) {
				...
				.then(function(result) {
					if (result) {
						const url  = "/quiz/admExrcsQstnsCmptnModifyAjax.do";
						const data = {
							exrcsSddnQstnBscId 	: "${vo.exrcsSddnQstnBscId}",
							searchGubun			: type
						};
						...
						ajaxCall(url, data, function (data) {
				            if (data.result > 0) {
				        		exrcsSddnViewMv("${vo.exrcsSddnQstnBscId}", "ADMEXRCSMODIFY", "EXRCS_QSTN", ($("#sbjctId").val() || "${vo.sbjctId}"));	// 관리자 연습문제 수정 화면
				            } else {
				                UiComm.showMessage(data.message, "error");
				            }
				        }, function (xhr, status, error) {
                    ...
                    <div class="sub-content qstn">
				        ...
				        <div class="board_top">
					        <div class="right-area">
					            ...
					            <a href="javascript:exrcsSddnViewMv('', 'ADMEXRCSLIST', 'EXRCS_QSTN')" class="btn type2 big"><spring:message code="common.button.list" /></a><!-- 목록 -->
					        </div>
				        </div>
				        <!-- table-type-->
					        	...
					        	<table class="table-type5">
							        ...
							        <tbody>
							        	...
							        	<tr>
							        		...
							        		<td>
							        			<div class="inputDiv">
				                                    <select class="form-select wide" id="sbjctId" name="sbjctId" required="true">
				                                        <option value=""><spring:message code="common.subject" /><!-- 과목 --></option>
				                                    </select>
							        			</div>
							        			<div class="viewDiv">
							        				<c:out value="${vo.sbjctnm }" /> <c:out value="${vo.dvclasNo }" />
							        			</div>
							        		</td>
							        	</tr>
							        	<tr>
