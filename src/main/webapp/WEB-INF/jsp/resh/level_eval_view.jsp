...
<script type="text/javascript">
	...
	function viewReshList() {
		var url  = "/resh/Form/levelEvalList.do";
		var form = $("<form></form>");
		        	...
		        	<div class="layout2">
						...
						<div id="info-item-box">
	                    	...
	                    	<h2 class="page-title flex-item flex-wrap gap4 columngap16">
		                    	<spring:message code="resh.label.learning.activity" /><!-- 학습활동 -->
		                    </h2>
		                	<div class="button-area">
			               	<c:if test="${menuType.contains('STUDENT') and reshVO.reschStatus eq '진행' and reshVO.joinYn eq 'N' and PROFESSOR_VIRTUAL_LOGIN_YN ne 'Y'}">
			                    <a href="javascript:reshJoinPop('<c:out value="${reshVO.reschCd}" />')" class="ui blue button"><spring:message code="resh.button.level.eval.join" /></a><!-- 만족도조사 참여 -->
			               	</c:if>
			               	<c:if test="${menuType.contains('STUDENT') and reshVO.reschStatus eq '진행' and reshVO.joinYn eq 'Y' and PROFESSOR_VIRTUAL_LOGIN_YN ne 'Y'}">
			                    <a href="javascript:reshJoinPop('<c:out value="${reshVO.reschCd}" />')" class="ui blue button"><spring:message code="resh.button.level.eval.edit" /></a><!-- 만족도조사 수정 -->
			               	</c:if>
				                            ...
				                            <div class="title_cont">
				                                <div class="left_cont">
				                                    <div class="lectTit_box">
				                                        ...
				                                        <span class="fcGrey"><small><spring:message code="resh.label.level.eval.period" /><!-- 만족도조사 기간 --> : <uiex:formatDate value="${reshVO.reschStartDttm}" pattern="yyyy.MM.dd HH:mm" /> ~ <uiex:formatDate value="${reshVO.reschEndDttm}" pattern="yyyy.MM.dd HH:mm" /></small></span>
				                                    </div>
				                                </div>
				                            </div>
				                        ...
				                        <div class="content menu_off active">
				                        	<ul class="tbl-simple">
				                        		<li>
													<dl>
														<dt>
															<label for="subjectLabel"><spring:message code="resh.label.level.eval.content" /></label><!-- 만족도조사 내용 -->
														</dt>
														<dd></dd>
													</dl>
												...
												<li>
													<dl>
														<dt>
															<label for="teamLabel"><spring:message code="resh.label.level.eval.qstn" /></label><!-- 만족도조사 문항 -->
														</dt>
												...
												<li>
													<dl>
														...
														<c:forEach items="${reshVO.fileList}" var="row">
															<button class="ui icon small button" title="<spring:message code="forum.label.attachFile.download" />" type="button" onclick="fileDown('<c:out value="${row.fileSn}" />', '<c:out value="${row.repoCd}" />')">
