...
<html lang="ko" style="position: fixed; width: 100%;">
	...
	<script type="text/javascript">
		...
		var excelExampleGrid = {
			colModel:[
		        ...
		        {label:"<spring:message code='resh.label.point' />",	name:'score',	align:'right',	width:'5000',	colums:'D'}/* 점수 */
			]
		};
		
		// 등록
		function uploadExcelReshScore(fileObj, copyFile) {
			showLoading();
            $.ajax({
                  type: 'post',
                  url: '/resh/uploadReshScoreExcel.do',
                  async: false,
                  dataType: "json",
                  data: {
                          "reschCd"    	  : "${vo.reschCd}"
		...
		function sampleExcelDown() {
			...
			$("#scoreUploadForm").attr("target", "exampleExcelDownloadIfm");
	        $("#scoreUploadForm").attr("action", "/resh/quizScoreSampleDownload.do");
	    	...
	    	ajaxCall(url, data, function(data) {
	    		if(data.result > 0) {
	    	    	...
	    	    	var copyFile = fileUploader.getCopyFiles();
	    	 		...
	    	 		uploadExcelReshScore(fileObj, copyFile);
	    		} else {
	...
	<form id="scoreUploadForm" name="scoreUploadForm" method="POST">
