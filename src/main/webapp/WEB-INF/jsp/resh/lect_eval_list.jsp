...
<script type="text/javascript">
	...
	function createLectEvalListHTML(list, pageInfo) {
		...
		if(isList) {
			...
			list.forEach(function(v, i) {
				...
				listHtml += '	<td><a href="javascript:void(0)" class="fcBlue" onclick="moveLectEvalView(\'' + v.reschCd + '\')">' + v.reschTitle + '</a></td>';
				...
				if(isStudent) {
					...
					if(v.reschStatus == '진행' && v.joinYn == 'N' && PROFESSOR_VIRTUAL_LOGIN_YN != "Y") {
						listHtml += '		<a href="javascript:void(0)" class="ui blue button small" onclick="reshJoinPop(\'' + v.reschCd + '\')"><spring:message code="resh.button.lect.eval.join" /></a>'; // 강의평가 참여
					}
					if(v.reschStatus == '진행' && v.joinYn == 'Y' && PROFESSOR_VIRTUAL_LOGIN_YN != "Y") {
						listHtml += '		<a href="javascript:void(0)" class="ui blue button small" onclick="reshJoinPop(\'' + v.reschCd + '\')"><spring:message code="resh.button.lect.eval.edit" /></a>'; // 강의평가 수정
					}
						listHtml += '	</td>';
				}
				listHtml += '</tr>';
			...
			html += '			<th><spring:message code="resh.label.lect.eval.name" /></th>'; 									// 강의평가명
			html += '			<th data-breakpoints="xs"><spring:message code="resh.label.lect.eval.period" /></th>'; 	// 강의평가 기간
			...
			if(list.length == 0 ){
				...
				list.forEach(function(v, i) {
					...
					listHtml += '			<label class="ui yellow label active"><spring:message code="resh.label.lect.eval" /></label>'; // 강의평가
					listHtml += '			<a href="javascript:void(0)" class="header header-icon" onclick="moveLectEvalView(\'' + v.reschCd + '\')">' + v.reschTitle + '</a>';
					listHtml += '		</div>';
					listHtml += '	</div>';
					listHtml += '	<div class="sum-box">';
					listHtml += '		<ul class="process-bar">';
					if(v.reschStatus == '대기') {
						listHtml += '			<li class="bar-softgrey"><spring:message code="resh.label.lect.eval" /> <spring:message code="resh.label.ready" /></li>'; // 강의평가 대기
					} else if(v.reschStatus == '진행') {
						listHtml += '			<li class="bar-blue"><spring:message code="resh.label.lect.eval" /> <spring:message code="resh.label.inprogress" /></li>'; // 강의평가 진행
					} else if(v.reschStatus == '완료') {
						listHtml += '			<li class="bar-softgrey"><spring:message code="resh.label.lect.eval" /> <spring:message code="resh.label.complete" /></li>'; // 강의평가 완료
					}
					listHtml += '		</ul>';
					listHtml += '	</div>';
					...
					listHtml += '				<label class="label-title-lg"><spring:message code="resh.label.lect.eval.period" /></label>'; // 강의평가 기간
				...
				if(isStudent && v.reschStatus == '진행' && v.joinYn == 'N' && PROFESSOR_VIRTUAL_LOGIN_YN != "Y") {
					listHtml += '				<a href="javascript:void(0)" class="ui blue button w150" onclick="reshJoinPop(\'' + v.reschCd + '\')"><spring:message code="resh.button.lect.eval.join" /></a>'; // 강의평가 참여
				}
				if(isStudent && v.reschStatus == '진행' && v.joinYn == 'Y' && PROFESSOR_VIRTUAL_LOGIN_YN != "Y") {
					listHtml += '				<a href="javascript:void(0)" class="ui blue button w150" onclick="reshJoinPop(\'' + v.reschCd + '\')"><spring:message code="resh.button.lect.eval.edit" /></a>'; // 강의평가 수정
				}
					listHtml += '			</div>';
					listHtml += '		</div>';
					listHtml += '	</div>';
	...
	function setList() {
		...
		$("#lectEvalList").empty().html(html);
		
		if($("#lectEvalList > table").length != 0) {
			// 리스트형 footable 적용
			$("#lectEvalList > table").footable();
		} else {
			// 카드형 dropdown 적용
			$("#lectEvalList").find(".ui.dropdown").dropdown();
		}
	...
	// 상세보기 이동
	function moveLectEvalView(reschCd) {
		var url  = "/resh/Form/lectEvalView.do";
		var form = $("<form></form>");
			...
			<div class="content stu_section">
		        ...
		        <div class="ui form">
		        	<div class="layout2">
                        ...
                        <div class="row">
                            <div class="col">
                            	<div class="ui warning message tc">
				                	<spring:message code="resh.label.lect.eval.period" /><!-- 강의평가 기간 -->
				                </div>
			                	<!-- 검색조건 -->
								<div class="option-content mb10">
									...
									<div class="ui action input search-box" style="max-width: calc(100% - 45px)">
										<input id="searchValue" type="text" placeholder="<spring:message code="resh.label.lect.eval.name" />" value="${param.searchValue}" /><!-- 강의평가명 -->
	    ...
	    <div class="modal-dialog modal-lg" role="document">
	        <div class="modal-content">
	            <div class="modal-header">
	                ...
	                <h4 class="modal-title"><spring:message code="resh.button.lect.eval.join" /><!-- 강의평가 참여 --></h4>
	            </div>
