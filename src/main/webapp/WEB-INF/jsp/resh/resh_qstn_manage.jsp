...
<script type="text/javascript">
	...
	var formOption = {
		...
		addHeader: function(reschPageCd) {
			var html  = "<div class='row wmax reshAddForm' id='"+reschPageCd+"QstnForm'>";
				html += "	<div class='col'>";
				...
				html += "				<div class='mla'>";
				html += "					<a href='javascript:saveReshQstn(\""+reschPageCd+"\", \"write\")' id='"+reschPageCd+"WriteBtn' class='ui blue button'><spring:message code='resh.button.save' /></a>";/* 저장 */
	    ...
	    scaleLvlChg: function(reschPageCd, itemList) {
	    	...
	    	var html = "";
	    	for(var i = cnt; i >= 1; i--) {
	    		var listCnt = itemList != null && cnt == 5 ? i - 1 : i;
		    	html += "<dl>";
			    html += "	<dt>";
        		...
        		if(pageList.length > 0) {
        			...
        			pageList.forEach(function(v, i) {
        				var pageTitle = escapeHtml(v.reschPageTitle);
	        				html += `<div class='grid-content-box menu-nth-wrap quizQstnList reschPage page_\${v.reschPageCd}' data-pageCd="\${v.reschPageCd}" data-pageOdr="\${v.reschPageOdr}">`;
	        				...
	        				html += "		</div>";
	        				html += "	</div>";
	        				html += `	<div class='mt10 sub-content wmax pl15 sort-qstn\${i} ui-sortable'>`;
        				v.reschQstnList.forEach(function(vv, ii) {
        					...
        					html += "				<i class='arrows alternate vertical icon icon-chg ui-sortable-handle'></i>";
    ...
    function addQstnForm(reschPageCd) {
    	...
    	$("#"+reschPageCd+"WriteBtn").attr("href", "javascript:saveReshQstn(\""+reschPageCd+"\", 'write')");
    }
    ...
    function editQstnForm(reschPageCd, reschQstnCd) {
    	...
    	$("#"+reschPageCd+"WriteBtn").attr("href", "javascript:saveReshQstn(\""+reschPageCd+"\", 'edit')");
    ...
    // 설문 문항 저장, 수정
    function saveReshQstn(reschPageCd, type) {
    	if(!validateWriteReshQstnItem(reschPageCd)) {
    		return false;
    	}
    	
    	showLoading();
    	var url = type == "write" ? "/resh/writeReshQstn.do" : "/resh/editReshQstn.do";
    ...
    function updateReshSubmitYn(type) {
    	if(type == "edit") {
    		var isJoin = "${vo.reschJoinUserCnt}" > 0;
    		if(isJoin) {
    			alert("<spring:message code='resh.alert.start.not.edit' />")/* 설문에 참여자가 있어 수정할 수 없습니다. */
	    		return false;
    		}
    	}
        		...
        		<div class="ui form">
        			<div class="layout2">
						<script>
							$(document).ready(function () {
								// set location
								setLocationBar('<spring:message code="resh.label.resh" />', '<small class="section"><spring:message code="resh.tab.item.manage" />'); // 설문 문항 관리
							});
						</script>
		                ...
		                <div id="info-item-box">
                            ...
                            <div class="button-area">
								...
								<a href="javascript:manageResh(9)" class="ui basic button"><spring:message code="resh.button.list" /></a><!-- 목록 -->
                            </div>
		                </div>
		                <div class="row">
		                	<div class="col">
		                		<div class="listTab">
			                        <ul>
			                            ...
			                            <li class="mw120"><a onclick="manageResh(2)"><spring:message code="resh.tab.result" /></a></li><!-- 설문정보 및 평가 -->
			                        </ul>
			                    </div>
