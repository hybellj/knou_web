...
<script type="text/javascript">
	...
	function list() {
		...
		tableHtml += '			</div>';
		tableHtml += '		</th>';
		tableHtml += '		<th class="tc" rowspan="2"><spring:message code="common.number.no" /><!-- No. --></th>';
		tableHtml += '		<th class="tc" rowspan="2"><spring:message code="score.label.univ.gbn1" /><!-- 계열/대학원 --></th>';
		tableHtml += '		<th class="tc" rowspan="2"><spring:message code="common.dept_name" /><!-- 학과 --></th>';
		tableHtml += '		<th class="tc" rowspan="2"><spring:message code="score.label.crs.cd" /><!-- 학수번호 --></th>';
		tableHtml += '		<th class="tc" rowspan="2"><spring:message code="score.label.crs.cre.nm" /><!-- 과목명 --></th>';
		tableHtml += '		<th class="tc" rowspan="2"><spring:message code="common.label.decls.no" /><!-- 분반 --></th>';
		tableHtml += '		<th class="tc" rowspan="2"><spring:message code="common.teaching.assistant" /><br /><spring:message code="score.label.hr.no" /><!-- 조교 인사번호 --></th>';
		tableHtml += '		<th class="tc" rowspan="2"><spring:message code="common.teaching.assistant" /><!-- 조교 --></th>';
		...
		var url = "/score/scoreMgr/listOprScoreAssistTotal.do";
		var param = {
			  haksaYear		: $("#haksaYear").val()
			, haksaTerm		: $("#haksaTerm").val()
			...
			if(data.result > 0) {
				...
				returnList.forEach(function(v, i) {
					...
					TERM_LESSON_LIST.forEach(function(vv, j) {
						html += '<td class="tc"><a href="javascript:oprScoreAssistWriteModal(\'' + v.crsCreCd + '\', \'' + v["lessonScheduleId" + vv.lsnOdr] + '\', \'' + v.userId + '\')" class="fcBlue">' + (typeof v["plusScore" + vv.lsnOdr] === "undefined" || v["plusScore" + vv.lsnOdr] === null ? '-' : v["plusScore" + vv.lsnOdr]) + '</a></td>';
						html += '<td class="tc"><a href="javascript:oprScoreAssistWriteModal(\'' + v.crsCreCd + '\', \'' + v["lessonScheduleId" + vv.lsnOdr] + '\', \'' + v.userId + '\')" class="fcBlue">' + (typeof v["minusScore" + vv.lsnOdr] === "undefined" || v["plusScore" + vv.lsnOdr] === null ? '-' : v["minusScore" + vv.lsnOdr]) + '</a></td>';
					});
					html += '	<td class="tc"><a href="javascript:oprScoreAssistTotalModal(\'' + v.crsCreCd + '\', \'' + v.userId + '\')" class="fcBlue">' + (typeof v.plusScore === "undefined" ? '-' : v.plusScore) + '</a></td>';
					html += '	<td class="tc"><a href="javascript:oprScoreAssistTotalModal(\'' + v.crsCreCd + '\', \'' + v.userId + '\')" class="fcRed">' + (typeof v.minusScore === "undefined" ? '-' : v.minusScore) + '</a></td>';
	...
	// 조교 수업운영 평가등록 모달
	function oprScoreAssistWriteModal(crsCreCd, lessonScheduleId, userId) {
		$("#oprScoreAssistWriteForm > input[name='crsCreCd']").val(crsCreCd);
		$("#oprScoreAssistWriteForm > input[name='lessonScheduleId']").val(lessonScheduleId);
		$("#oprScoreAssistWriteForm > input[name='userId']").val(userId);
		$("#oprScoreAssistWriteForm").attr("target", "oprScoreAssistWriteIfm");
        $("#oprScoreAssistWriteForm").attr("action", "/score/scoreMgr/oprScoreAssistWritePop.do");
        $("#oprScoreAssistWriteForm").submit();
        $('#oprScoreAssistWriteModal').modal('show');
        
        $("#oprScoreAssistWriteForm > input[name='crsCreCd']").val("");
        $("#oprScoreAssistWriteForm > input[name='lessonScheduleId']").val("");
		$("#oprScoreAssistWriteForm > input[name='userId']").val("");
	}
	
	// 조교 수업운영 평가등록 모달(전체보기)
	function oprScoreAssistTotalModal(crsCreCd, userId) {
		$("#oprScoreAssistWriteForm > input[name='crsCreCd']").val(crsCreCd);
		$("#oprScoreAssistWriteForm > input[name='userId']").val(userId);
		$("#oprScoreAssistWriteForm").attr("target", "oprScoreAssistWriteIfm");
        $("#oprScoreAssistWriteForm").attr("action", "/score/scoreMgr/oprScoreAssistWritePop.do");
	...
	function receiveMessage(event) {
 	...
 	// 조교 수업운영 평가 등록 팝업 콜백
 	function oprScoreAssistWritePopCallback() {
 		list();
 	}
	...
	function downExcel() {
		...
		var excelGrid = {
		    colModel:[
	            ...
	            {label:'<spring:message code="common.teaching.assistant.hr.no" />', name:'userId', align:'center', width:'5000'},// 조교 인사번호
		...
		var url  = "/score/scoreMgr/downExcelOprScoreAssistTotal.do";
		var form = $("<form></form>");
...
<body>
            ...
            <div class="content">
                <div id="info-item-box">
                	<h2 class="page-title flex-item">
					    ...
					    <div class="ui breadcrumb small">
					        <small class="section"><spring:message code='score.label.assist.oper.total' /><!-- 수업운영 점수전체(조교) --></small>
					    </div>
					</h2>
				</div>
				...
				<div class="ui form">
					...
					<div class="ui segment searchArea">
	   				...
	   				<div class="option-content gap4">
	   					<h3 class="sec_head"><spring:message code='score.label.assist.oper.total' /><!-- 수업운영 점수전체(조교) --></h3>
	   					...
	   					<table class="tBasic" data-empty="<spring:message code='common.content.not_found' />" id="operateScoreTable"><!-- 등록된 내용이 없습니다. -->
		  					...
		  					<thead>
			    				<tr>
			    					...
			    					<th class="tc" colspan="16">수업운영점수</th>
			    				</tr>
			    				<tr>
			    					<c:forEach begin="1" end="15" var="num">
			    						<th class="tc" style="min-width: 70px">${num}주</th>
			    					</c:forEach>
    ...
    <!-- 조교 수업운영 평가기준 팝업 -->
    <form id="oprScoreAssistWriteForm" name="oprScoreAssistWriteForm">
		<input type="hidden" name="crsCreCd" value="" />
    	<input type="hidden" name="lessonScheduleId" value="" />
    	<input type="hidden" name="userId" value="" />
	</form>
    <div class="modal fade in" id="oprScoreAssistWriteModal" tabindex="-1" role="dialog" aria-labelledby="<spring:message code="score.label.assist.oper.criteria"/>" aria-hidden="false" style="display: none; padding-right: 17px;">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <iframe src="" width="100%" id="oprScoreAssistWriteIfm" name="oprScoreAssistWriteIfm"></iframe>
                </div>
            </div>
        </div>
    </div>
