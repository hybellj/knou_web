	...
	<script type="text/javascript">
		...
		function changeUnivGbn(univGbn) {
			...
			this["CRS_CRE_LIST_" + ACTIVE_TAB].forEach(function(v, i) {
				if((univGbn == "ALL" || v.univGbn == univGbn) && v.deptId) {
					deptCdObj[v.deptId] = true;
				}
			});
			...
			USER_DEPT_LIST.forEach(function(v, i) {
				if(deptCdObj[v.deptId]) {
					html += '<option value="' + v.deptId + '">' + v.deptNm + '</option>';
				}
			});
			
			// 부서 초기화
			$("#deptId" + ACTIVE_TAB).html(html);
			$("#deptId" + ACTIVE_TAB).dropdown("clear");
			$("#deptId" + ACTIVE_TAB).on("change", function() {
		...
		function listLessonStatusByCrs() {
			...
			*/
			...
			var param = {
				...
				, deptId		: ($("#deptId1").val() || "").replace("ALL", "")
			...
			ajaxCall(url, param, function(data) {
				if(data.result > 0) {
					...
					SEARCH_OBJ_1.deptNm = SEARCH_OBJ_1.deptId ? $("#deptId1 > option:selected")[0].innerText : '<spring:message code="common.all" />';;
	        	} else {
	        		alert(data.message);
	        	}
			}, function(xhr, status, error) {
		...
		function downExcel1() {
			...
			form.append($('<input/>', {type: 'hidden', name: 'deptId', 		value: SEARCH_OBJ_1.deptId}));
		...
		function listLessonStatusByStd() {
			...
			var param = {
				...
				, deptId		: ($("#deptId2").val() || "").replace("ALL", "")
			...
			ajaxCall(url, param, function(data) {
				if(data.result > 0) {
					...
					SEARCH_OBJ_2.deptNm = SEARCH_OBJ_2.deptId ? $("#deptId2 > option:selected")[0].innerText : '<spring:message code="common.all" />';
		...
		function downExcel2() {
			...
			form.append($('<input/>', {type: 'hidden', name: 'deptId', 		value: SEARCH_OBJ_2.deptId}));
		...
		function listAdminDashUser(pageIndex) {
			...
			var data = {
				...
				, deptId			: ($("#deptId3").val() || "").replace("ALL", "")
			...
			<div class="content">
				<div class="ui divider mt0"></div>
					...
					<div class="ui form">
						...
						<!-- 전체공지, 전체설문 -->
						<div class="ui stackable grid mt0">
							<div class="eight wide column pt0 flex1 flex flex-column">
								<div class="ui attached message">
									<div class="header flex">
										<span class="mra"><spring:message code="bbs.label.system_notice" /></span><!-- 전체공지 -->
										<a href="/bbs/bbsMgr/atclList.do?bbsId=${sysNoticeId}" aria-label="<spring:message code="common.label.view.more"/>" class="flex-item f080"><i class="plus icon"></i></a><!-- 더보기 -->
									</div>
								</div>
								<%-- 
								<div class="ui attached segment flex flex-column flex1">
								<ul class="list_line_S2">
									<c:forEach var="notice" items="${sysNoticeList}">
										<li>
											<a href="/bbs/bbsHome/Form/atclView.do?bbsId=${notice.bbsId}&atclId=${notice.atclId}" class="justify-content-between">
												<div class="flex flex-column">
                                                    <div class="flex">
                                                        <p class=""><c:out value="${notice.atclTitle}"/></p>
                                                        <c:if test="${notice.isNew eq 'Y'}">
															<i class="text-new flex-none"><spring:message code="bbs.label.new_atcl" /></i><!-- 새글 -->
														</c:if>
                                                    </div>
                                                    <small class="m0 p0"><c:out value="${notice.regNm}"/> | <c:out value="${notice.regDttm}"/></small>
												</div>
								...
								<c:if test="${empty sysNoticeList}">
									<div class="ui attached segment flex flex-column flex1">
		                                <div class="flex-item-center flex1">
		                                	<!-- 등록된 내용이 없습니다. -->
		                                    <spring:message code="common.content.not_found"/>
		                                </div>
		                            </div>
				                </c:if>
								</div>
							...
							<div class="eight wide column pt0 flex1 flex flex-column">
								<div class="ui attached message">
									<div class="header flex">
										<span class="mra"><spring:message code="resh.label.resh.home" /></span><!-- 전체설문 -->
										<a href="/resh/reshMgr/Form/homeReshList.do" aria-label="<spring:message code="common.label.view.more"/>" class="flex-item f080"><i class="plus icon"></i></a><!-- 더보기 -->
									</div>
								</div>
							</div>
							 --%>
						...
						<div class="ui segment">
							<div class="ui pointing secondary tabmenu menu">
								<a class="item pt0 active" data-tab="1" onclick="changeTab('1')"><spring:message code="dashboard.label.lesson.status.crs" /><!-- 과목별 학습현황 --></a>
								<a class="item pt0" data-tab="2" onclick="changeTab('2')"><spring:message code="dashboard.label.lesson.status.std" /><!-- 학생별 학습현황 --></a>
								<a class="item pt0" data-tab="3" onclick="changeTab('3')"><spring:message code="crs.title.letcuser.search" />	</a><!-- 사용자 검색 -->
							</div>
								...
								<%-- <div class="option-content gap4">
									...
									<select class="ui dropdown" id="haksaYear1" onchange="changeTerm()">
				                   	...
				                   	<c:if test="${orgId eq 'ORG0000001'}">
										<!-- 대학구분 -->
			                            <select id="univGbn1" class="ui dropdown">
			                    			...
			                    			<c:forEach items="${univGbnList}" var="row">
	                            	...
	                            	<select id="crsCreCd1" class="ui dropdown mr5 w250">
			                    		<option value=""><spring:message code="common.subject" /><!-- 과목 --> <spring:message code="common.select" /><!-- 선택 --></option>
			                    	</select>
									<div class="ui action input search-box">
										<input id="searchValue1" type="text" placeholder="<spring:message code="contents.label.crscrenm" />/<spring:message code="common.label.prof.nm" />/<spring:message code="contents.label.crscd" /> <spring:message code='button.search'/>" />
										<a class="ui icon button" onclick="listLessonStatusByCrs()"><i class="search icon"></i></a>
								    </div>
								    <div class="mla">
								    	<uiex:msgSendBtn func="sendMsg('crs')" styleClass="ui basic button"/><!-- 메시지 -->
				   						<a href="javascript:void(0)" class="ui green button" onclick="downExcel1()"><spring:message code="common.button.excel_down" /><!-- 엑셀 다운로드 --></a>
								    </div>
								</div> --%>

								<div class="">
									<div id="lessonStatusByCrsTable" ></div>
									<script>
										var lessonStatusByCrsTable;
										if("<%=SessionInfo.isKnou(request)%>" == "true") {
											lessonStatusByCrsTable = new Tabulator("#lessonStatusByCrsTable", {
				                        		maxHeight: "600px",
				                        		minHeight: "100px",
				                        		layout: "fitColumns",
				                        		...
				                        		columns: [
				                        			{formatter:"rowSelection", titleFormatter:"rowSelection", headerHozAlign:"center", hozAlign:"center", vertAlign:"middle", width:40, headerSort:false, cellClick:function(e,cell){cell.getRow().toggleSelect()}},
				                        		    {title:"<spring:message code='common.number.no'/>", field:"lineNo", width:40, headerHozAlign:"center", hozAlign:"center", vertAlign:"middle", formatter:"rownum", headerSort:false},
				                        		    {title:"<spring:message code='common.type'/>", field:"univGbnNm", headerHozAlign:"center", hozAlign:"center", vertAlign:"middle", width:80, headerSort:false}, // 구분				                        		    
				                        		    {title:"<spring:message code='std.label.dept'/>", field:"deptNm", headerHozAlign:"center", vertAlign:"middle", minWidth:150, headerSort:false}, // 학과
				                        		    {title:"<spring:message code='common.crs.cd'/>", field:"crsCd", headerHozAlign:"center", hozAlign:"center", vertAlign:"middle", minWidth:80, headerSort:false}, // 학수번호
				                        		    {title:"<spring:message code='review.label.crscrenm'/> (<spring:message code='review.label.decls'/>)", field:"crsCreNm", headerHozAlign:"center", vertAlign:"middle", minWidth:200, formatter:"html", headerSort:false, frozen:true}, // 과목명 (분반)
				                        		    {title:"<spring:message code='common.professor'/>", field:"userNm", headerHozAlign:"center", hozAlign:"center", vertAlign:"middle", minWidth:80, headerSort:false}, // 교수
				                        		    {title:"<spring:message code='common.teaching.assistant'/>", field:"assistInfo", headerHozAlign:"center", hozAlign:"center", vertAlign:"middle", minWidth:120, formatter:"html", headerSort:false}, // 조교
				                        		    {title:"<spring:message code='std.label.learner'/>", field:"stdCnt", headerHozAlign:"center", hozAlign:"center", vertAlign:"middle", minWidth:60, headerSort:false}, // 수강생
				                        		    {title:"<spring:message code='std.label.auditor'/>", field:"auditCnt", headerHozAlign:"center", hozAlign:"center", vertAlign:"middle", minWidth:60, headerSort:false}, // 청강생
													{title:"<spring:message code='common.label.class.plan'/>", field:"lsnPlanUrl", headerHozAlign:"center", hozAlign:"center", vertAlign:"middle", minWidth:80, formatter:"html", headerSort:false}, // 수업계획서
													{title:"<spring:message code='common.label.lec.announcement'/>", field:"noticeCnt", headerHozAlign:"center", hozAlign:"center", vertAlign:"middle", width:70, headerSort:false}, // 강의공지
				                        		    {title:"<spring:message code='dashboard.qna'/>", field:"qnaCnt", headerHozAlign:"center", hozAlign:"center", vertAlign:"middle", width:70, formatter:"html", headerSort:false}, // Q&A
				                        		    {title:"<spring:message code='dashboard.councel'/>", field:"secretCnt", headerHozAlign:"center", hozAlign:"center", vertAlign:"middle", width:70, formatter:"html", headerSort:false}, // 1:1상담
				                        		    {title:"<spring:message code='common.label.tasks'/><br/><spring:message code='common.label.evaluation'/>", field:"asmntCnt", headerHozAlign:"center", hozAlign:"center", vertAlign:"middle", width:70, formatter:"html", headerSort:false}, // 과제평가
				                        		    {title:"<spring:message code='common.label.discussion'/><br/><spring:message code='common.label.evaluation'/>", field:"forumCnt", headerHozAlign:"center", hozAlign:"center", vertAlign:"middle", formatter:"html", width:70, headerSort:false}, // 토론평가
				                        		    {title:"<spring:message code='common.label.question'/><br/><spring:message code='common.label.evaluation'/>", field:"quizCnt", headerHozAlign:"center", hozAlign:"center", vertAlign:"middle", width:70, formatter:"html", headerSort:false}, // 퀴즈평가
				                        		    {title:"<spring:message code='common.label.resh'/><br/><spring:message code='common.label.evaluation'/>", field:"reschCnt", headerHozAlign:"center", hozAlign:"center", vertAlign:"middle", width:70, formatter:"html", headerSort:false}, // 설문평가
													{title:"<spring:message code='dashboard.cor.always.exam'/>", field:"aexamCnt", headerHozAlign:"center", hozAlign:"center", vertAlign:"middle", width:70, formatter:"html", headerSort:false}, // 수시평가
				                        		    {title:"<spring:message code='dashboard.exam_mid'/>", field:"midExamInfo", headerHozAlign:"center", hozAlign:"center", vertAlign:"middle", width:150, formatter:"html", headerSort:false}, // 중간고사
				                        		    {title:"<spring:message code='dashboard.exam_end'/>", field:"lastExamInfo", headerHozAlign:"center", hozAlign:"center", vertAlign:"middle", width:150, formatter:"html", headerSort:false} // 기말고사
				                        		]
			                        		});
										} else {
											lessonStatusByCrsTable = new Tabulator("#lessonStatusByCrsTable", {
				                        		...
				                        		columns: [
				                        			{formatter:"rowSelection", titleFormatter:"rowSelection", headerHozAlign:"center", hozAlign:"center", vertAlign:"middle", width:40, headerSort:false, cellClick:function(e,cell){cell.getRow().toggleSelect()}},
				                        		    {title:"<spring:message code='common.number.no'/>", field:"lineNo", width:40, headerHozAlign:"center", hozAlign:"center", vertAlign:"middle", formatter:"rownum", headerSort:false},
				                        		    {title:"<spring:message code='common.type'/>", field:"uniNm", headerHozAlign:"center", hozAlign:"center", vertAlign:"middle", width:70, headerSort:false}, // 구분				                        		    
				                        		    {title:"<spring:message code='std.label.dept'/>", field:"deptNm", headerHozAlign:"center", vertAlign:"middle", minWidth:70, headerSort:false}, // 학과
				                        		    {title:"<spring:message code='common.crs.cd'/>", field:"crsCd", headerHozAlign:"center", hozAlign:"center", vertAlign:"middle", minWidth:100, headerSort:false}, // 학수번호
				                        		    {title:"<spring:message code='review.label.crscrenm'/> (<spring:message code='review.label.decls'/>)", field:"crsCreNm", headerHozAlign:"center", vertAlign:"middle", minWidth:320, formatter:"html", headerSort:false, frozen:true}, // 과목명 (분반)
				                        		    {title:"<spring:message code='common.professor'/>", field:"userNm", headerHozAlign:"center", hozAlign:"center", vertAlign:"middle", minWidth:80, headerSort:false}, // 교수
				                        		    {title:"<spring:message code='common.teaching.assistant'/>", field:"assistInfo", headerHozAlign:"center", hozAlign:"center", vertAlign:"middle", minWidth:120, formatter:"html", headerSort:false, widthShrink: 1}, // 조교
				                        		    {title:"<spring:message code='std.label.learner'/>", field:"stdCnt", headerHozAlign:"center", hozAlign:"center", vertAlign:"middle", minWidth:60, headerSort:false}, // 수강생
				                        		    {title:"<spring:message code='std.label.auditor'/>", field:"auditCnt", headerHozAlign:"center", hozAlign:"center", vertAlign:"middle", minWidth:60, headerSort:false}, // 청강생
													{title:"<spring:message code='common.label.lec.announcement'/>", field:"noticeCnt", headerHozAlign:"center", hozAlign:"center", vertAlign:"middle", width:90, headerSort:false}, // 강의공지
				                        		    {title:"<spring:message code='dashboard.qna'/>", field:"qnaCnt", headerHozAlign:"center", hozAlign:"center", vertAlign:"middle", width:90, formatter:"html", headerSort:false}, // Q&A
				                        		    {title:"<spring:message code='dashboard.councel'/>", field:"secretCnt", headerHozAlign:"center", hozAlign:"center", vertAlign:"middle", width:90, formatter:"html", headerSort:false}, // 1:1상담
				                        		    {title:"<spring:message code='common.label.tasks'/><br/><spring:message code='common.label.evaluation'/>", field:"asmntCnt", headerHozAlign:"center", hozAlign:"center", vertAlign:"middle", width:90, formatter:"html", headerSort:false}, // 과제평가
				                        		    {title:"<spring:message code='common.label.discussion'/><br/><spring:message code='common.label.evaluation'/>", field:"forumCnt", headerHozAlign:"center", hozAlign:"center", vertAlign:"middle", formatter:"html", width:90, headerSort:false}, // 토론평가
				                        		    {title:"<spring:message code='common.label.question'/><br/><spring:message code='common.label.evaluation'/>", field:"quizCnt", headerHozAlign:"center", hozAlign:"center", vertAlign:"middle", width:90, formatter:"html", headerSort:false}, // 퀴즈평가
				                        		    {title:"<spring:message code='common.label.resh'/><br/><spring:message code='common.label.evaluation'/>", field:"reschCnt", headerHozAlign:"center", hozAlign:"center", vertAlign:"middle", width:90, formatter:"html", headerSort:false}, // 설문평가
				                        		]
			                        		});
										}
			                        </script>
							...
							<div class="ui tab" data-tab="2">
								<%-- <div class="option-content gap4">
									...
									<select class="ui dropdown" id="haksaYear2" onchange="changeTerm()">
				                   	...
				                   	<c:if test="${orgId eq 'ORG0000001'}">
									<!-- 대학구분 -->
		                            <select id="univGbn2" class="ui dropdown" >
		                    			<option value=""><spring:message code="common.label.uni.type" /></option><!-- 대학구분 -->
		                    			<option value="ALL"><spring:message code="common.all" /></option><!-- 전체 -->
		                    			<c:forEach items="${univGbnList}" var="row">
		                    				<option value="${row.codeCd}">${row.codeNm}</option>	
	                        		...
	                        		<!-- 학과 선택 -->
	                            	<select id="deptCd2" class="ui dropdown w250">
	                                	<option value=""><spring:message code="common.dept_name" /><!-- 학과 --> <spring:message code="common.select" /><!-- 선택 --></option>
	                            	</select> 
	                            	<select id="crsCreCd2" class="ui dropdown mr5 w250">
			                    		<option value=""><spring:message code="common.subject" /><!-- 과목 --> <spring:message code="common.select" /><!-- 선택 --></option>
			                    	</select>
									<div class="ui action input search-box">
										<input id="searchValue2" type="text" placeholder="<spring:message code='dashboard.user_name'/>/<spring:message code='common.label.student.number'/> <spring:message code='button.search'/>" />
										<a class="ui icon button" onclick="listLessonStatusByStd()"><i class="search icon"></i></a>
								    </div>
								    <div class="mla">
								    	<uiex:msgSendBtn func="sendMsg('std')" styleClass="ui basic button"/><!-- 메시지 -->
				   						<a href="javascript:void(0)" class="ui green button" onclick="downExcel2()"><spring:message code="common.button.excel_down" /><!-- 엑셀 다운로드 --></a>
								    </div>
								</div> --%>
							
								<div class="">
									<div id="lessonStatusStdTable"></div>
									<script>
										// 학습현황 목록 테이블
				                        var lessonStatusStdTable = new Tabulator("#lessonStatusStdTable", {
				                        		maxHeight: "600px",
				                        		minHeight: "100px",
				                        		layout: "fitColumns",
				                        		selectableRows: "highlight",
				                        		...
				                        		columns: [
				                        			{formatter:"rowSelection", titleFormatter:"rowSelection", headerHozAlign:"center", hozAlign:"center", vertAlign:"middle", width:40, headerSort:false, cellClick:function(e,cell){cell.getRow().toggleSelect();}},
				                        		    {title:"<spring:message code='common.number.no'/>", field:"lineNo", width:40, headerHozAlign:"center", hozAlign:"center", vertAlign:"middle", formatter:"rownum", headerSort:false},
				                        		    {title:"<spring:message code='common.label.userdept.grade'/>", field:"hy", headerHozAlign:"center", hozAlign:"center", vertAlign:"middle", minWidth:60}, // 학년
				                        		    {title:"<spring:message code='common.label.entrance'/>", field:"enterDt", headerHozAlign:"center", hozAlign:"center", vertAlign:"middle", minWidth:70}, // 입학
				                        		    {title:"<spring:message code='common.name'/>", field:"userNm", headerHozAlign:"center", vertAlign:"middle", minWidth:90, formatter:"html"}, // 이름
				                        		    {title:"<spring:message code='common.label.student.number'/>", field:"userId", headerHozAlign:"center", hozAlign:"center", vertAlign:"middle", minWidth:100}, //  학번
				                        		    {title:"<spring:message code='common.dept_name'/>", field:"deptNm", headerHozAlign:"center", vertAlign:"middle", minWidth:150}, // 학과
				                        		    {title:"<spring:message code='common.subject'/>", field:"crsCreNm", headerHozAlign:"center", vertAlign:"middle", minWidth:150, formatter:"html"}, // 과목
				                        		    {
				                        		    	title:"<spring:message code='dashboard.attend.status'/>", headerHozAlign:"center",
				                        		    	columns: [
	                        		    					{title:"<div class='weekTop'>1</div><div class='weekBot'>9</div>", field:"week1", headerHozAlign:"center", hozAlign:"center", width:40, headerSort:false, formatter:"html"},
						                        		    {title:"<div class='weekTop'>2</div><div class='weekBot'>10</div>", field:"week2", headerHozAlign:"center", hozAlign:"center", width:40, headerSort:false, formatter:"html"},
						                        		    {title:"<div class='weekTop'>3</div><div class='weekBot'>11</div>", field:"week3", headerHozAlign:"center", hozAlign:"center", width:40, headerSort:false, formatter:"html"},
						                        		    {title:"<div class='weekTop'>4</div><div class='weekBot'>12</div>", field:"week4", headerHozAlign:"center", hozAlign:"center", width:40, headerSort:false, formatter:"html"},
						                        		    {title:"<div class='weekTop'>5</div><div class='weekBot'>13</div>", field:"week5", headerHozAlign:"center", hozAlign:"center", width:40, headerSort:false, formatter:"html"},
						                        		    {title:"<div class='weekTop'>6</div><div class='weekBot'>14</div>", field:"week6", headerHozAlign:"center", hozAlign:"center", width:40, headerSort:false, formatter:"html"},
						                        		    {title:"<div class='weekTop'>7</div><div class='weekBot'>/</div>", field:"week7", headerHozAlign:"center", hozAlign:"center", width:40, headerSort:false, formatter:"html"}
				                        		    	],
				                        		    },
				                        		    {title:"<spring:message code='dashboard.prog'/>", field:"studyRate", headerHozAlign:"center", hozAlign:"center", vertAlign:"middle", minWidth:80, formatter:"html"}, // 진도율
				                        		    {title:"<spring:message code='dashboard.cor.exam'/>", field:"examCnt", headerHozAlign:"center", hozAlign:"center", vertAlign:"middle", minWidth:60, formatter:"html", headerSort:false}, // 시험
				                        		    {title:"<spring:message code='dashboard.qna'/>", field:"qnaCnt", headerHozAlign:"center", hozAlign:"center", vertAlign:"middle", minWidth:60, formatter:"html", headerSort:false}, // Q&A
				                        		    {title:"<spring:message code='dashboard.cor.councel'/>", field:"secretCnt", headerHozAlign:"center", hozAlign:"center", vertAlign:"middle", minWidth:60, formatter:"html", headerSort:false}, // 1:1
				                        		    {title:"<spring:message code='common.label.asmnt'/>", field:"asmntCnt", headerHozAlign:"center", hozAlign:"center", vertAlign:"middle", minWidth:60, formatter:"html", headerSort:false}, // 과제
				                        		    {title:"<spring:message code='common.label.forum'/>", field:"forumCnt", headerHozAlign:"center", hozAlign:"center", vertAlign:"middle", minWidth:60, formatter:"html", headerSort:false}, // 토론
				                        		    {title:"<spring:message code='common.label.question'/>", field:"quizCnt", headerHozAlign:"center", hozAlign:"center", vertAlign:"middle", minWidth:60, formatter:"html", headerSort:false}, // 퀴즈
				                        		    {title:"<spring:message code='common.label.resh'/>", field:"reschCnt", headerHozAlign:"center", hozAlign:"center", vertAlign:"middle", minWidth:60, formatter:"html", headerSort:false}, // 설문
				                        		    {title:"<spring:message code='dashboard.cor.admission'/>", field:"aexamCnt", headerHozAlign:"center", hozAlign:"center", vertAlign:"middle", minWidth:60, formatter:"html", headerSort:false}, // 수시
				                        		    {title:"<spring:message code='dashboard.seminar'/>", field:"seminarCnt", headerHozAlign:"center", hozAlign:"center", vertAlign:"middle", minWidth:60, formatter:"html", headerSort:false}, // 세미나
				                        		]
			                        	});
			                        </script>									
								</div>
							...
							<div class="ui tab" data-tab="3">
	                           	<%-- <div class="option-content gap4">
									...
									<select class="ui dropdown" id="haksaYear3" onchange="changeTerm()">
				                   	...
				                   	<!-- 대학구분 -->
				                   	<input type="hidden" id="uniCd3" />
	                        		<!-- 학과 선택 -->
	                            	<select id="deptCd3" name="deptId" class="ui dropdown w250" >
	                                	<option value=""><spring:message code="user.title.userdept.select" /></option>
	                            	</select> 
									<div class="ui action input search-box">
										<input id="searchValue3" type="text" placeholder="<spring:message code="user.title.userinfo.user.no" /> <spring:message code='button.search'/>" /> <!-- 학번/학과명/이름 입력 -->
										<a class="ui icon button" onclick="listAdminDashUser(1);"><i class="search icon"></i></a>
									</div>
								</div> --%>
								
								<div class="">
									<div id="adminDashUserTable"></div>
									<script>
										// 사용자 목록 테이블
				                        var adminDashUserTable = new Tabulator("#adminDashUserTable", {
				                        		maxHeight: "600px",
				                        		minHeight: "100px",
				                        		layout: "fitColumns",
				                        		selectableRows: "highlight",
				                        		...
				                        		placeholder:"<spring:message code='common.content.not_found'/>",
				                        		columns: [
				                        		    {title:"<spring:message code='common.number.no'/>", field:"lineNo", width:60, headerHozAlign:"center", hozAlign:"center", formatter:"rownum", headerSort:false},
				                        		    {title:"<spring:message code='common.type'/>", field:"authGrpNm", headerHozAlign:"center", hozAlign:"center", width:70, headerSort:false}, // 구분				                        		    
				                        		    {title:"<spring:message code='user.title.userdept.dept'/>", field:"deptNm", headerHozAlign:"center", minWidth:150, headerSort:false}, // 학과/부서
				                        		    {title:"<spring:message code='user.title.userinfo.user.no'/>", field:"userId", headerHozAlign:"center", hozAlign:"center", minWidth:80, headerSort:false}, //  학번/사번
				                        		    {title:"<spring:message code='common.name'/>", field:"userNm", headerHozAlign:"center", minWidth:100, formatter:"html", headerSort:false}, // 이름
				                        		    {title:"<spring:message code='common.label.userdept.grade'/>", field:"userGrade", headerHozAlign:"center", hozAlign:"center", minWidth:70, headerSort:false}, // 학년
				                        		    //{title:"<spring:message code='user.title.userinfo.phoneno'/>", field:"mobileNo", headerHozAlign:"center", minWidth:100, headerSort:false}, // 전화번호
				                        		    //{title:"<spring:message code='common.email'/>", field:"email", headerHozAlign:"center", minWidth:120, headerSort:false}, // 이메일
				                        		    {title:"<spring:message code='user.title.userinfo.user.stats'/>", field:"schregGbn", headerHozAlign:"center", hozAlign:"center", minWidth:70, headerSort:false}, // 학적상태
				                        		    <%
													if (!SessionInfo.isKnou(request)) {
														%>
														{title:"한사대 계정 연결", field:"linkHycu", headerHozAlign:"center", hozAlign:"center", minWidth:120, formatter:"html", headerSort:false}, // 한사대 계정 연결
														<%
													}
													%>
													{title:"<spring:message code='user.button.login'/>", field:"login", headerHozAlign:"center", hozAlign:"center", minWidth:80, formatter:"html", headerSort:false}, // 로그인
				                        		]
			                        	});
			                        </script>
								</div>
