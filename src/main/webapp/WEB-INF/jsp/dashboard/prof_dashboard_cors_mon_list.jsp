...
<c:forEach var="cors" items="${monCorsList}" varStatus="status">
    <section class="item">
        <div class="header">
            <a href="<c:url value='/crs/crsHomeProf.do'/>?crsCreCd=${cors.crsCreCd}" title="강의실 이동"
               class="h4 <c:if test='${cors.uniCd eq "G"}'>graduSch</c:if>"><c:if
                    test="${cors.univGbn eq '3'}">[<spring:message code='crs.label.general'/><!-- 일반 -->] </c:if>${cors.crsCreNm}
                (${cors.declsNo}<spring:message code="dashboard.cor.dev_class"/>)</a>
            <div class="extra1"> <!-- 수강 청강 태그 변경 및 위치 이동_230517 -->
                <dl>
                    <dt><spring:message code="common.professor"/></dt>
                    <dd>${empty cors.profNm ? '-' : cors.profNm}</dd>
                </dl> <!-- 교수 -->
                <dl>
                    <dt><spring:message code="lesson.lecture"/></dt>
                    <dd>${cors.stdCnt}<spring:message code="resh.label.nm"/></dd>
                </dl> <!-- 수강, 명 -->
                <dl>
                    <dt><spring:message code="lesson.auditor"/></dt>
                    <dd>${cors.auditCnt}<spring:message code="resh.label.nm"/></dd>
                </dl><!-- 청강, 명 -->
                <dl>
                    <dt>${cors.assistNm}</dt>
            ...
            <div class="cnt">
                ...
                <!-- 과제평가 -->
                   <c:choose>
                       <c:when test="${cors.asmntSbmtCnt > 0}">
                           <a class="box" data-label="<spring:message code='asmt.button.asmt.eval' />" href="javascript:moveCrs('/asmt/profAsmtListView.do?crsCreCd=<c:out value="${cors.crsCreCd}"/>')"><span>${cors.asmntEvalCnt}<small>${cors.asmntSbmtCnt}</small></span></a>
                       </c:when>
                       <c:otherwise>
                ...
                <!-- 토론평가 -->
                   <c:choose>
                       <c:when test="${cors.forumAnsCnt > 0}">
        ...
        <div class="extra2">
            <dl class="flex1"
                ...
                onclick="moveCrs('/exam/Form/examList.do?crsCreCd=${cors.crsCreCd}&examType=EXAM')"
                    ...
                    <c:when test="${not empty cors.examMid && exam.scoreRatio gt 0}">
                        <c:choose>
                            ...
                            <c:when test="${not empty exam.examStartDttm && exam.examTypeCd ne 'EXAM' }">
                                <dd><spring:message code="common.etc"/>(${exam.examTypeNm})</dd>
                                <!-- 기타 -->
                            </c:when>
                            ...
                            <c:when test="${empty exam.examStartDttm && exam.examTypeCd ne 'EXAM' }">
                                <dd><spring:message code="common.etc"/>(<spring:message code="common.ready"/>)</dd>
                                <!-- 기타 (준비 중) -->
                            </c:when>
                        </c:choose>
...
<c:if test="${empty monCorsList}">
    <div class="flex-container" style="min-height:300px">
        <!-- 등록된 내용이 없습니다. -->
        <div class="cont-none fcGrey"><i class="icon-list-no ico"></i><span class="fcGrey"><spring:message
                code="common.content.not_found"/></span></div>
    </div>
</c:if>
