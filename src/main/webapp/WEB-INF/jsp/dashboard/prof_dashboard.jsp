...
<body class="home ${uiex:getTheme()} ">
	...
	function loadWidgetContent(widgetId) {
		switch (widgetId) {
			case 'wigt_prof_today': // Today
				dashboardWidget.loadUrl(widgetId, "/dashboard/profWidgetToday.do?encParams="+encParams);
				break;

			case 'wigt_prof_schedule': // 이달의 학사일정
				dashboardWidget.loadUrl(widgetId, "/dashboard/profWidgetSchedule.do?encParams="+encParams);
				break;

			case 'wigt_prof_notice': // 공지사항
				dashboardWidget.loadUrl(widgetId, "/dashboard/profWidgetNotice.do?encParams="+encParams);
				break;

			case 'wigt_prof_qna': // 강의Q&A
				dashboardWidget.loadUrl(widgetId, "/dashboard/profWidgetQna.do?encParams="+encParams);
				break;

			case 'wigt_prof_counsel': // 1:1상담
				dashboardWidget.loadUrl(widgetId, "/dashboard/profWidgetCounsel.do?encParams="+encParams);
				break;

			case 'wigt_prof_msg': // 알림(메시지)
				dashboardWidget.loadUrl(widgetId, "/dashboard/profWidgetMsg.do?encParams="+encParams);
				break;

			case 'wigt_prof_subject': // 강의과목
				dashboardWidget.loadUrl(widgetId, "/dashboard/profWidgetSubject.do?encParams="+encParams);
				break;
	...
	//게시글 보기클릭시 글로벌메뉴로 이동
	function viewAtclDash(atclId, rgtrId, oyn, bbsId, bbsTycd, upMenuId, menuId ) {
		let extData = {
			atclId	: atclId,
			rgtrId 	: rgtrId,
			oyn 	: oyn,
		...
		document.location.href = "/bbs/bbsHome/bbsAtclView.do?encParams="+EPARAM+"&addParams="+UiComm.makeEncParams(extData);
	}
    </script>
