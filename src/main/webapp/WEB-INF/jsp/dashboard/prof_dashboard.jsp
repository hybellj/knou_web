...
<link rel="stylesheet" type="text/css" href="/webdoc/css/main_default.css?v=3" />
    ...
    <body class="<%=SessionInfo.getThemeMode(request)%>">
        <div id="wrap" class="main">
			...
			<%
			// 기관:한사대
            /* if (SessionInfo.isLmsBasic(request)) { */
				%>
                        ...
                        <%@ include file="/WEB-INF/jsp/common/frontGnb.jsp" %>
                            ...
                            <main class="common">
                                ...
                                <div class="col-left"> 
                                    ...
                                    <ul class="infoIconSet" style="<%=(SessionInfo.getAuthrtGrpcd(request).contains("ADM") == true ? "_display:none" : "")%>">
                                        <li class="info bcPurpleAlpha85 <%if(!SessionInfo.isLmsBasic(request)){%>bcPeruBlue90<%}%>">
                                            <div class="flex-item">
                                                <div class="alt_icon">
                                        ...
                                        <li class="info bcDarkblueAlpha85 <%if(!SessionInfo.isLmsBasic(request)){%>bcPeruBlue70<%}%>">    
                                            <div class="flex-item">                                        
                                    ...
                                    <div class="component33 off" data-medi-ui="more-off" data-ctrl-off="더보기">
                                        ...
                                        <div class="listTab2 noti" data-target="tab_contents1">
                                        	<div data-target="#tab1" class="active">
                                        		<c:choose>
	                                        		<c:when test="${isLmsBasic eq 'true'}"><spring:message code="dashboard.notice_all"/><!-- 전체공지 --></c:when>
	                                        		<c:otherwise><spring:message code="dashboard.notice_course"/><!-- 강의공지 --></c:otherwise>
	                                        	</c:choose>
                                       		</div>
                                            <div data-target="#tab2">
                                            	<c:choose>
	                                        		<c:when test="${isLmsBasic eq 'true'}"><spring:message code="dashboard.notice_course"/><!-- 강의공지 --></c:when>
                                        ...
                                        <div class="tab_contents1"> 
                                            <div id="tab1" class="tab_content on">
                                                <c:choose>
                                                	<c:when test="${isLmsBasic eq 'true'}">
	                                    ...
	                                    var stuDashMsg = {
			                                	init: function() {
			                                		...
			                                		$("[data-dash-tab]").off("click").on("click", function () {
			                                		    ...
			                                		    $("#dash" + dashTab + "List").show();
			                                		    $("[data-dash-tab]").removeClass("active");
			                                		    $(this).addClass("active");
			                                		});
			                                	},
			                                	loadAlimData: function() {
			                                		var param = { chnlCd: 'ALL', listCnt: 5 };

			                                		ajaxCall('/msg/alim/alimChnlListAjax.do', param, function(data) {
			                                			if (data.result > 0 && data.returnVO) {
			                                				var returnVO = data.returnVO;
			                                				// 개수 업데이트
			                                				if (returnVO.unreadCnt) {
			                                					stuDashMsg.updateCnt(returnVO.unreadCnt);
			                                				}
			                                				// 목록 렌더링
			                                				stuDashMsg.renderList('Push', returnVO.pushList, '#dashPushList', 'P');
			                                				stuDashMsg.renderList('Sms', returnVO.smsList, '#dashSmsList', 'S');
			                                				stuDashMsg.renderList('Msg', returnVO.msgList, '#dashMsgList', 'M');
			                                				stuDashMsg.renderList('Talk', returnVO.talkList, '#dashTalkList', 'T');
			                                			}
			                                		}, function(xhr, status, error) {
			                                			console.error('알림 데이터 조회 실패');
			                                		}, false, {type: "GET"});
			                                	},
			                                	updateCnt: function(data) {
			                                		$('#dashPushCount').text(data.pushCnt || 0);
			                                		$('#dashSmsCount').text(data.smsCnt || 0);
			                                		$('#dashMsgCount').text(data.shrtntCnt || 0);
			                                		$('#dashTalkCount').text(data.alimtalkCnt || 0);
			                                	},
			                                	renderList: function(tabName, list, targetSelector, type) {
			                                		var $target = $(targetSelector);
			                                		var html = '';
			                                		if (list && list.length > 0) {
			                                			$.each(list, function(idx, item) {
			                                				if (idx >= 5) return false;
			                                				var opacityClass = (item.readYn === 'Y') ? 'opacity7' : '';
			                                				var title = item.sndngTtl || '';
			                                				var date = UiComm.formatDate(item.sndngDttm, "datetime");
			                                				html += '<div class="item flex-row p8 ' + opacityClass + '">';
			                                				html += '    <div class="flex overflow-hidden">';
			                                				html += '        <a href="javascript:stuDashMsg.moveMsgList(\'' + type + '\')" class="f100 overflow-hidden" title="내용보기">';
			                                				html += '            <p class="item-title mb5 text-truncate">' + UiComm.escapeHtml(title) + '</p>';
			                                				html += '            <small class="f080 fcGrey">' + date + '</small>';
			                                				html += '        </a>';
			                                				html += '    </div>';
			                                				html += '</div>';
			                                			});
			                                			...
			                                			html = '<div class="flex-container m-hAuto">';
			                                			html += '    <div class="no_content">';
			                                			html += '        <span>수신한 메시지가 없습니다.</span>';
			                                			html += '        <a href="javascript:stuDashMsg.moveMsgList(\'' + type + '\')" class="ui basic small button mt5">수신함 보기</a>';
			                                			html += '    </div>';
			                                			html += '</div>';
			                                		}
			                                		$target.html(html);
										...
										if (SessionInfo.isLmsBasic(request)) {
	                                        ...
	                                        <div class="option-content">
	                                            ...
	                                            <div class="button-area">
	                                            	<c:set var="mType">
	                                            		<%=(SessionInfo.getAuthrtCd(request).contains("TUT") == true ? "ASSISTANT" : "PROF") %>
	                                            	</c:set>
                            ...
                            <script type="text/javascript">
                            	...
                            	function evalPop(type) {
                            		...
                            		$("#evalModal h4.modal-title").text(type == "PROF" ? "<spring:message code='dashboard.professor.class.eval.crit' />" : "<spring:message code='score.label.assist.oper.criteria' />"); // 교수 수업운영 평가기준, 조교 수업운영 평가기준
                            		var form = $("<form></form>");
                            		form.attr("method", "POST");
                            		form.attr("name", "tempForm");
                            		form.attr("action", type == "PROF" ? "/score/scoreMgr/oprScoreProfCriteriaPop.do" : "/score/scoreMgr/oprScoreAssistCriteriaPop.do");
