...
//		getForumContsView("", "ALL");
		...
		getForumContsView($(selUser).attr("data-userId"), $(selUser).attr("data-stdNo"));
	}

	$("#selectedUserId").val($(selUser).attr("data-userId"));
	$("#selectedUserNm").val($(selUser).attr("data-userNm"));
	$("#selectedStdNo").val($(selUser).attr("data-stdNo"));
...
function toggleJoinUser(obj) {
	if ($(obj).hasClass("select")) {
		...
		getForumContsView($(obj).attr("data-userId"), $(obj).attr("data-stdNo"));
...
function getTargetUserInfoAndScore(obj) {
	getTotalScoreInputView($(obj).attr("data-userId"), $(obj).attr("data-stdNo"));
	getTargetUserInfoView($(obj).attr("data-userId"), $(obj).attr("data-stdNo"));
	getForumFeedbackView($(obj).attr("data-userId"), $(obj).attr("data-stdNo"));
}
</script>
...
<c:if test="${not empty resultList}">
	...
	<c:forEach items="${resultList }" var="item" varStatus="status">
		...
		<a href="javascript:;" name="ezgTargetUser" onClick="toggleJoinUser(this)" data-userId="${item.userId}" data-userId="${item.userId}" data-userNm="${item.userNm}" data-stdNo="${item.stdNo}" class="card active-toggle-btn ${item.joinStatus == 'JOIN'?'submit':''} ${vo.stdNo == item.stdNo?'select':''}">
