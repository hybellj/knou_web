...
function selectTeam(obj) {
	...
	getForumContsView(null, "ALL", $(obj).attr("data-teamCd"), $(obj).attr("data-teamStdNos"));
...
function selectUser(obj) {
	...
	$("#selectedStdNo").val($(obj).attr("data-stdNo"));
	$("#selectedTeamCd").val($(obj).attr("data-teamCd"));
	getForumContsView($(obj).attr("data-userId"), $(obj).attr("data-stdNo"), $(obj).attr("data-teamCd"), $(obj).attr("data-teamStdNos"));
	getTargetUserInfoAndScore(obj)
}

// 제줄자의 평가점수 및 유저 정보 조회
function getTargetUserInfoAndScore(obj) {
	getTotalScoreInputView($(obj).attr("data-userId"), $(obj).attr("data-stdNo"), $(obj).attr("data-teamCd"));
	getTargetUserInfoView($(obj).attr("data-userId"), $(obj).attr("data-stdNo"));
	getForumFeedbackView($(obj).attr("data-userId"), $(obj).attr("data-stdNo"), $(obj).attr("data-teamCd"));
}
</script>
<input type="hidden" id="selectedUserId" value="" />
<input type="hidden" id="selectedUserNm" value="" />
<input type="hidden" id="selectedStdNo" value="" />
<input type="hidden" id="selectedTeamCd" value="" />
<input type="hidden" id="ezgForumCtgrCd" value="${forumVO.forumCtgrCd}" />
<c:if test="${not empty resultList}">
	<c:forEach items="${resultList }" var="item" varStatus="status">
		<a href="javascript:;" name="ezgTargetUser" onClick="selectTeam(this)" data-teamCd="${item.teamCd}" data-teamStdNos="${item.teamStdNos}" class="ui grey label m0 tr active-toggle-btn flex-none ${vo.teamCd == item.teamCd?'select':''}">
		...
		<c:if test="${not empty item.teamMembers}">
			<c:forEach items="${item.teamMembers }" var="team" varStatus="teamStatus">
				<a href="javascript:;" name="ezgTargetUser" onClick="toggleJoinUser(this)" data-userId="${team.userId}" data-userId="${team.userId}" data-userNm="${team.userNm}" data-stdNo="${team.stdNo}" data-teamCd="${item.teamCd}" data-teamStdNos="${item.teamStdNos}" class="card active-toggle-btn ${team.joinStatus == 'JOIN'?'submit':''} ${vo.stdNo == team.stdNo?'select':''}">
					<div class="content stu_card">
