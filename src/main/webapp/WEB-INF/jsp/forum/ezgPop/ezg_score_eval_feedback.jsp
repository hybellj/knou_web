...
<c:if test="${forumVo.forumCtgrCd eq 'NORMAL' && CLASS_USER_TYPE ne 'CLASS_LEARNER'  && (empty forumVo.searchMenu || forumVo.searchMenu != 'EZG') && forumVo.evalCritUseYn eq 'N'}">
	...
	<div class="flex-item flex-wrap gap4 pt10">
		...
		<a href="javascript:void(0);" class="ui basic fluid button" onClick="fdbkList('${forumVo.stdNo}')" id="cntFdbk">${cntFdbk}<spring:message code='forum.label.cnt.feedback'/><!-- 개의 피드백 --></a>
		</c:if>
	</div>
	</c:if>
