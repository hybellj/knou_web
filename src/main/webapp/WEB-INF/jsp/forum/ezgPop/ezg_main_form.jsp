...
function getJoinUserOrTeamSearchView() {
	var url = "/forum/ezgPop/ezgJoinUserSearchView.do";
	var paramData = {
		"crsCreCd" : $("#ezgCrsCreCd").val()
		, "forumCd" : $("#ezgForumCd").val()
	}; 
...
function getJoinUserOrTeamList() {
	var url = "/forum/ezgPop/joinUserList.do";
...
function listForum(page, userId, stdNo, teamStdNos) {
	...
	$("#forumContBlock").load(
		"/forum/forumLect/evalForumBbsViewList.do"
		, {
			"pageIndex" : page,
			"listScale" : listScale,
...
// 전체 점수 입력 화면 로드
function getTotalScoreInputView(userId, stdNo, teamCd) {
	//	if (!stdNo && !teamCd) {
	...
	if("${vo.evalCtgr}" == "R") {
		var url = "/forum/ezgPop/ezgTotalScoreView.do";
		var paramData = {
			"crsCreCd" : $("#ezgCrsCreCd").val()
			, "forumCd" : $("#ezgForumCd").val()
			, "userId" : userId
		...
		$("#totalScoreInputBlock").load(
			url
			, paramData
			, function () {getEvalScoreInputView(userId, stdNo, teamCd);}
		);
	} else {
	*/
		var url = "/forum/ezgPop/ezgScoreView.do";
		var paramData = {
			"crsCreCd" : $("#ezgCrsCreCd").val()
			, "forumCd" : $("#ezgForumCd").val()
			, "userId" : userId
...
function getEvalScoreInputView(userId, stdNo, teamCd) {
	...
	var url = "/forum/ezgPop/ezgEvalScoreView.do";
	var paramData = {
		"crsCreCd" : $("#ezgCrsCreCd").val()
		, "forumCd" : $("#ezgForumCd").val()
		, "userId" : userId
...
// 제줄자 정보 화면 로드
function getTargetUserInfoView(userId, stdNo) {
	if (!stdNo) {
...
// feedback 화면 로드
function getForumFeedbackView(userId, stdNo, teamCd) {
	if(stdNo) {
		$("#forumFeedbackBlock").load(
			"/forum/ezgPop/forumScoreEvalFeedBack.do"
			, {
				"crsCreCd" : $("#ezgCrsCreCd").val()
				, "forumCd" : $("#ezgForumCd").val()
				, "userId" : userId
...
function reloadFeedbackView() {
	getForumFeedbackView($("#selectedUserId").val(), $("#selectedStdNo").val(), $("#selectedTeamCd").val());
}
...
function addFdbkCts(forumFdbkCd, parForumFdbkCd){
	...
	if(fdbkCts == null || fdbkCts == "") {
		...
		if(forumFdbkCd == "" ||forumFdbkCd == null ) {
			$.getJSON("/forum/forumLect/addFdbkCts.do", {
				"fdbkCts" : fdbkCts,
			...
			$.getJSON("/forum/forumLect/editFdbkCts.do", {
				"fdbkCts" : fdbkCts,
...
function addFCmntCts(index,forumFdbkCd,parForumFdbkCd) {
	...
	if(fCmnt == null || fCmnt == "") {
		...
		if(forumFdbkCd == "" ||forumFdbkCd == null ) {
			...
			$.getJSON("/forum/forumHome/editFdbkCts", {
				"fdbkCts" : fCmnt,
				"forumFdbkCd" : forumFdbkCd,
				"forumCd" : $("#ezgForumCd").val(),
				"stdNo" : $("#selectedStdNo").val(),
...
function delFdbk(forumFdbkCd) {
	...
	$("#alertOk").on('click', function(e) {
		$.getJSON("/forum/forumLect/delFdbkCts.do", {
			"forumFdbkCd" : forumFdbkCd,
			"stdNo" : $("#selectedStdNo").val(),
			"forumCd" : $("#ezgForumCd").val(),
			"userId" : $("#selectedUserId").val(),
