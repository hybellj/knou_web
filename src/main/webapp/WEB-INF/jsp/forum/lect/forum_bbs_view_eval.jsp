						...
						<c:forEach var="item" items="${forumAtclVO.cmntList}" varStatus="cmnt">
							...
							<ul>
								...
								<li>
									<ul>
										<li class="flex-item">
											<em class="mra">${item.regNm} (${item.cmntCtsLen}<spring:message code='forum.label.word'/><!-- 자 -->) <code>${regDate}</code></em>
									...
									<ul>
										...
										<li>
											...
											<c:choose>
												...
												<c:otherwise>
												<em class="mra">@${item.parRegNm}</em> <div id="cmntCts${cmnt.index}${status.index}">${item.cmntCts}</div>
												</c:otherwise>
											</c:choose>
											</div>
										</li>
