...
<head>
	...
	<%@ include file="/WEB-INF/jsp/common/common_inc.jsp"%>
   	...
   	<script type="text/javascript">
			...
			ajaxCall(url, data, function(data) {
				if(data.result > 0) {
					...
					listLessonTime.forEach(function(lessonTime, i) {
						...
						if(lessonTime.stdyMethod == "RND") {
							stdyMethodNm = '<spring:message code="lesson.label.stdy.method.rnd" />'; // 랜덤학습
						} else if(lessonTime.stdyMethod == "SEQ") {
							stdyMethodNm = '<spring:message code="lesson.label.stdy.method.seq" />'; // 순차학습
						}
						...
						html += '				<p class="lect_name">' + lessonTime.lessonTimeNm;
						html += '				<small class="ml5 fcGrey">';
						html += 					stdyMethodNm;
						html += '				</small>';
						html += '			</div>';
						html += '		</div>';
						...
						listLessonCnts.forEach(function(lessonCnts, j) {
							if(lessonCnts.prgrYn != "Y") return true;
							...
							listLessonStudyDetail.forEach(function(lessonStudyDetail, k) {
								listLessonStudyDetailTotal.push($.extend({
									  lessonTimeNm	: lessonTime.lessonTimeNm
									, lessonCntsNm	: lessonCnts.lessonCntsNm
									, cntsGbn		: lessonCnts.cntsGbn
								}, lessonStudyDetail));
							});
							
							html += '	<li class="mb5">';
							html += '		<div class="ui message flex">';
							html += '		<span class="ui blue circular label mr10" style="height: 30px">' + (lessonTime.stdyMethod == "SEQ" ? lessonCnts.lessonCntsOrder : '-') + '</span>';
							html += '			<div>';
							html += '				<div class="mb5">';
							html += '					<span class="fcGreen mr5">[' + lessonCnts.cntsGbn + ']</span>';
							html += 					lessonCnts.lessonCntsNm;
							html += '				</div>';
							html += '				<div class="flex f080 fcGrey">';
							html += '					<span>';
							html +=							'<spring:message code="lesson.label.lt.dttm" />: '; // 출석인정 기간
							html += 						lessonStartDtFmt;
							html += '						<span class="ml5 mr5">~</span>';
							html += 						lessonEndDtFmt;
							if(lessonCnts.prgrYn == "Y") {
								html += '					<span class="ml5 mr5">|</span>';
								html += '					<spring:message code="lesson.label.stdy.prgr.y" />'; // 출결대상
							}
							html += '					</span>';
							html += '				</div>';
							html += '			</div>';
					...
					// 주차명 설정
					$("#lessonScheduleNmText").text((returnVO.lessonScheduleNm || ""));
					...
					// 학습기록 세팅
					listLessonStudyDetailTotal = listLessonStudyDetailTotal.sort(function(a, b) {
					...
					var html = '';
					listLessonStudyDetailTotal.forEach(function(v, i) {
						...
						var studyTm = v.studyTm || 0;
	   		 			var mi = Math.floor(studyTm / 60);
	   		 			var sec = studyTm % 60;
	   		 			
						html += '<tr>';
						html += '	<td>' + (listLessonStudyDetailTotal.length - i) + '</td>';
						html += '	<td>' + v.lessonTimeNm + '</td>';
						html += '	<td>' + v.lessonCntsNm + '</td>';
						html += '	<td>' + regDttmFmt + '</td>';
						if(v.cntsGbn == "VIDEO" || v.cntsGbn == "VIDEO_LINK") {
							html += '<td>' + mi + '<spring:message code="lesson.label.min" /> ' + sec + '<spring:message code="lesson.label.sec" />' + '</td>'; // 분, 초
						} else {
							html += '<td>-</td>';
						}
					...
					//if(startDtYn == "Y" && returnVO.lessonScheduleEnd14Yn == "N" && (returnVO.wekClsfGbn == "02" || returnVO.wekClsfGbn == "03") || returnVO.prgrVideoCnt > 0) {
						...
						if(studyStatusCdBak && (returnVO.studyStatusCd == "COMPLETE" || returnVO.studyStatusCd == "LATE")) {
							...
							$("#attendBtn").popup({ popup : '.ui.flowing.popup.attendMemo', on : "click" });
						} else {
							$("#attendBtn").hide();
						}
					...
					$("#studyHistoryList2").html(html);
					$("#studyHistoryListTable2").footable();
		    	} else {
		    		alert(data.message);
		    	}
	   	...
	   	function reload() {
	 		var url = "/lesson/lessonPop/lessonStatusDetailPop.do";
	...
	<div id="wrap">
		<div class="ui form">
			<div class="fields">
                <div class="two wide field">
                    <div class="initial-img border-radius0" id="pthFileDiv" style="width:100px">
                        <img src="<c:out value="${empty stdVO.phtFile ? '/webdoc/img/icon-hycu-symbol-grey.svg' : stdVO.phtFile}" />" alt="사용자 이미지" />
                    </div>                                            
                </div>
			...
			<div class="ui segment">
	        	<div class="flex">
			        <p class="flex-item ml10" id="lessonScheduleNmText">
			        </p>
		        	<div class="flex-left-auto" id="attendBtnArea" style="display: none;">
		        		<a href="javascript:void(0)" class="ui blue button small" id="attendBtn"></a>
		        		<div class="ui flowing popup left center transition hidden attendMemo">
			                <form id="saveForm" name="saveForm">
				                ...
				                <div>
				                    ...
				                    <div class="tc mt20">
				                    	<button type="button" class="ui blue small button" onclick="saveForcedAttend()"><spring:message code="common.button.save" /><!-- 저장 --></button>
				                    </div>
				                </div>
			                </form>
			            </div>
	        ...
	        <div class="ui segment">
	        	<div style="max-height: 260px; overflow: auto;">
	        		<table id="studyHistoryListTable2" class="table type2" data-sorting="false" data-paging="false" data-empty="<spring:message code="common.content.not_found" />">
						<thead>
							<tr>
	        ...
	        <div class="ui segment">
	        	<div class="option-content mb10">
	        		<h3 class="mb5"><spring:message code="lesson.label.study.history" /><!-- 학습기록 --></h3>
		        </div>
		        <div style="max-height: 260px; overflow: auto;">
					<table id="studyHistoryListTable" class="table type2" data-sorting="false" data-paging="false" data-empty="<spring:message code="common.content.not_found" />">
						<thead>
							<tr>
								...
								<th scope="col" data-breakpoints="xs"><spring:message code="lesson.label.time" /><!-- 교시 --></th>
								<th scope="col" data-breakpoints="xs"><spring:message code="lesson.label.cnts.nm" /><!-- 콘텐츠명 --></th>
								<th scope="col"><spring:message code="lesson.label.study.reg.date" /><!-- 학습기록시간 --></th>
								<th scope="col"><spring:message code="lesson.label.study.time" /><!-- 학습시간 --></th>
								<th scope="col" data-breakpoints="xs"><spring:message code="lesson.label.type" /><!-- 구분 --></th>
								<th scope="col" data-breakpoints="xs"><spring:message code="lesson.label.reg.ip" /><!-- 접속 IP --></th>
							</tr>
						</thead>
