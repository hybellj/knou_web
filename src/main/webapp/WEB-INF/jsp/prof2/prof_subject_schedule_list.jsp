...
<div class="row week-list">
    <div class="col backcolorArea">
        ...
        <ul class="flex-tab week">
            <c:forEach var="lctr" items="${lctrSchdlList}" varStatus="status">
                <c:set var="ingClass" value=""/>
                <c:set var="emptyClass" value=""/>
                
                <c:if test="${lctr.lctrWknoSchdlId eq 'PROGRESS'}"><!-- 수정필요 -->
                    <c:set var="ingClass" value="state_now"/>
                </c:if>
                <c:if test="${lctr.lctrWknoSchdlId eq 'READY'}"><!-- 수정필요 -->
                    <c:set var="emptyClass" value="ico"/>
                </c:if>

                <li class="<c:out value="${ingClass}"/> state">
                    <a href="javascript:selectLessonSchedule('<c:out value="${lctr.lctrWknoSchdlId}"/>')">
           				<span class="week">${lctr.lctrWknoSeqno}</span>
                        <i id="lessonScheduleJoinRate_<c:out value="${lctr.lctrWknoSchdlId}"/>" class="<c:out value="${emptyClass}"/>"></i>
                    </a>
                </li>
            </c:forEach>
        </ul>
