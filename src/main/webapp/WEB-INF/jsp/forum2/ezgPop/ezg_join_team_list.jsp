...
<c:if test="${not empty resultList}">
	<c:forEach items="${resultList }" var="item" varStatus="status">
		<a href="javascript:;" name="ezgTargetUser" onClick="selectTeam(this)" data-team-id="${item.teamId}" data-teamStdIds="${item.teamStdIds}" data-dscs-id="${item.dscsId}" class="ui grey label m0 tr active-toggle-btn flex-none ${dscsVO.teamId == item.teamId?'select':''}">
		...
		<c:if test="${not empty item.teamMembers}">
			<c:forEach items="${item.teamMembers }" var="team" varStatus="teamStatus">
				<a href="javascript:;" name="ezgTargetUser" onClick="toggleJoinUser(this)" data-userId="${team.userId}" data-userId="${team.userId}" data-userNm="${team.userNm}" data-StdId="${team.stdId}" data-team-id="${item.teamId}" data-teamStdIds="${item.teamStdIds}" data-dscs-id="${item.dscsId}" class="card active-toggle-btn ${team.joinStatus == 'JOIN'?'submit':''} ${dscsVO.stdId == team.stdId?'select':''}">
					<div class="content stu_card">
