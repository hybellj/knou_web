...
<head>
    ...
    /**
     * createForumListHTML - 퍼블리싱 HTML 구조에 맞게 수정
     * 게시글(answer)과 댓글(Comment) 생성
     */
    function createForumListHTML(forumList, cmntList) {
        var html = '';
        ...
        if (forumList && forumList.length > 0) {
            $.each(forumList, function(idx, forum) {
                html += '<div class="answer">';
                html += '    <div class="title_area">';
                html += '        <strong class="title">' + escapeHtml(forum.TITLE || '') + '</strong>';
                html += '        <span class="date">';
                html += '            <b>' + escapeHtml(forum.WRITER_NAME || '작성자') + '</b>';
                html += '            <em>' + (forum.REG_DATE || '') + '</em>';
                html += '        </span>';
                html += '    </div>';
                html += '    <div class="cont">';
                html += '        ' + (forum.CONTENT || '').replace(/\n/g, '<br>');

                // 수정/삭제 버튼 (작성자 본인인 경우)
                if (forum.IS_WRITER === 'Y') {
                    html += '        <div class="bottom_btn">';
                    html += '            <div class="right-area">';
                    html += '                <button type="button" class="btn basic forumEdit" data-seq="' + forum.SEQ + '">수정</button>';
                    html += '                <button type="button" class="btn basic forumDelete" data-seq="' + forum.SEQ + '">삭제</button>';
                    html += '            </div>';
                    html += '        </div>';
                }
        ...
        // 2. 댓글 영역 (Comment)
        // ========================================
        var totalCommentCnt = cmntList ? cmntList.length : 0;

        html += '<div class="Comment">';
        html += '    <div class="top_area">';
        html += '        <button class="toggle_commentlist">';
        html += '            <i class="icon-svg-message"></i>' + totalCommentCnt + '개의 댓글이 있습니다. ';
        html += '            <i class="icon-svg-arrow-down"></i>';
        html += '        </button>';
        ...
        html += '            <fieldset>';
        html += '                <legend class="sr_only">댓글등록</legend>';
        html += '                <div class="memo">';
        ...
        html += '                            <a href="#0" class="simpleComment">수고했어요</a>';
        html += '                            <a href="#0" class="simpleComment">고생하셨어요.</a>';
        html += '                            <a href="#0" class="simpleComment">감사합니다.</a>';
        html += '                        </div>';
        html += '                    </div>';

        html += '                    <textarea title="댓글을 등록하세요." class="comment" id="newCommentContent" rows="3" cols="76" placeholder="댓글을 입력해 주세요"></textarea>';
        html += '                    <div class="bottom_btn">';
        html += '                        <span class="custom-input">';
        html += '                            <input type="checkbox" name="feedbackYn" id="feedbackLabel">';
        html += '                            <label for="feedbackLabel">피드백 문의 <span class="small">( 체크 시 문의로 등록되며 답변을 받을 수 있습니다. )</span></label>';
        html += '                        </span>';
        html += '                        <div class="right-area">';
        html += '                            <button type="button" class="btn type2 commentInsert">댓글 등록</button>';
        html += '                        </div>';
        html += '                    </div>';
        html += '                </div>';
        html += '            </fieldset>';
        ...
        if (cmntList && cmntList.length > 0) {
            // 최상위 댓글만 필터링
            var parentComments = $.grep(cmntList, function(item) {
                return !item.PARENT_SEQ || item.PARENT_SEQ === '' || item.PARENT_SEQ === '0';
            });

            $.each(parentComments, function(idx, comment) {
                html += '            <li>';
                html += '                <div class="item" data-seq="' + comment.SEQ + '">';
                html += '                    <div class="cmt_info">';
                html += '                        <strong class="name">' + escapeHtml(comment.WRITER_NAME || '익명') + '</strong>';
                html += '                        <span class="date">' + (comment.REG_DATE || '') + '</span>';
                html += '                    </div>';
                html += '                    <span class="comment">' + escapeHtml(comment.CONTENT || '').replace(/\n/g, '<br>') + '</span>';
                html += '                    <span class="cmtBtnGroup">';

                // 작성자 본인인 경우 수정/삭제 버튼
                if (comment.IS_WRITER === 'Y') {
                    html += '                        <button class="cmtUpt" data-seq="' + comment.SEQ + '">수정</button>';
                    html += '                        <button class="cmtDel" data-seq="' + comment.SEQ + '">삭제</button>';
                }

                // 답글 버튼
                html += '                        <button class="cmtWri" data-seq="' + comment.SEQ + '">댓글</button>';
                html += '                    </span>';
                html += '                </div>';

                // 답글 작성 폼 (숨김 상태)
                html += '                <div class="recmt_form" style="display:none;">';
                html += '                    <fieldset>';
                html += '                        <legend class="sr_only">댓글등록</legend>';
                html += '                        <div class="memo">';
                html += '                            <textarea title="댓글을 등록하세요." class="comment replyContent" rows="3" cols="76" placeholder="댓글을 입력해 주세요"></textarea>';
                html += '                            <button type="button" class="cmt_create replyInsert" data-parent-seq="' + comment.SEQ + '">댓글 등록</button>';
                html += '                        </div>';
                html += '                    </fieldset>';
                html += '                </div>';

                // 대댓글 목록
                var replies = $.grep(cmntList, function(item) {
                    return item.PARENT_SEQ === comment.SEQ;
                });

                if (replies.length > 0) {
                    html += '                <ul class="re_comment_ul">';

                    $.each(replies, function(ridx, reply) {
                        html += '                    <li class="re_comment">';
                        html += '                        <div class="item" data-seq="' + reply.SEQ + '">';
                        html += '                            <div class="cmt_info">';
                        html += '                                <strong class="name">' + escapeHtml(reply.WRITER_NAME || '익명') + '</strong>';
                        html += '                                <span class="date">' + (reply.REG_DATE || '') + '</span>';
                        html += '                            </div>';
                        html += '                            <span class="comment">' + escapeHtml(reply.CONTENT || '').replace(/\n/g, '<br>') + '</span>';
                        html += '                            <span class="cmtBtnGroup">';

                        // 작성자 본인인 경우 수정/삭제 버튼
                        if (reply.IS_WRITER === 'Y') {
                            html += '                                <button class="cmtUpt" data-seq="' + reply.SEQ + '">수정</button>';
                            html += '                                <button class="cmtDel" data-seq="' + reply.SEQ + '">삭제</button>';
                        }
            ...
            html += '            <li><div class="item"><span class="comment">등록된 댓글이 없습니다.</span></div></li>';
        }
    ...
    /**
     * 이벤트 바인딩 함수들
     * - 동적으로 생성된 HTML에 이벤트 연결
     */
    $(document).on('click', '.simpleComment', function(e) {
        e.preventDefault();
        var text = $(this).text();
        $('#newCommentContent').val(text);
        $(this).addClass('current').siblings().removeClass('current');
    });

    $(document).on('click', '.toggle_commentlist', function() {
        $(this).closest('.Comment').find('.comment_list').toggle();
        $(this).find('.icon-svg-arrow-down').toggleClass('rotate');
    });

    $(document).on('click', '.toggle_commentwrite', function() {
        $(this).closest('.Comment').find('.recmt_form').first().toggle();
    });

    $(document).on('click', '.cmtWri', function() {
        var $li = $(this).closest('li');
        $li.find('> .recmt_form').toggle();
    });

    $(document).on('click', '.commentInsert', function() {
        var content = $('#newCommentContent').val().trim();
        var feedbackYn = $('#feedbackLabel').is(':checked') ? 'Y' : 'N';
        ...
        // Ajax 댓글 등록 호출
        insertComment(content, feedbackYn);
    });

    $(document).on('click', '.replyInsert', function() {
        var parentSeq = $(this).data('parent-seq');
        var content = $(this).siblings('.replyContent').val().trim();
        ...
        // Ajax 답글 등록 호출
        insertReply(parentSeq, content);
    });

    $(document).on('click', '.cmtUpt', function() {
        var seq = $(this).data('seq');
        var $item = $(this).closest('.item');
        var currentContent = $item.find('.comment').text();

        // 수정 모드로 전환
        editComment(seq, currentContent, $item);
    });
    ...
    $(document).on('click', '.forumEdit', function() {
        var seq = $(this).data('seq');
        // 게시글 수정 처리
        editForum(seq);
    });
