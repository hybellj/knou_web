...
<head>
    ...
    <script type="text/javascript">
        ...
        function goRegist() {
       	 	...
       	 	UiComm.showMessage('등록하시겠습니까?', 'confirm').then(function(ok) {
                if(!ok) {
                    return;
                } else {
                	
		            $.ajax({
		                url : "/acad/schdl/admAcademicScheduleRegist.do",
		                type : "POST",
		                dataType : "json",
		                ...
		                if (data.result > 0 || data.successCount > 0) {
		                	...
		                	UiComm.showMessage(data.message || "등록에 실패하였습니다.");
		                }
		
		            }).fail(function(xhr) {
		            	UiComm.showMessage("등록 중 오류가 발생하였습니다.");
		            });
                }
       	 	});
        }
                ...
                <div class="sub-content">
                    ...
                    등록
                   	<div class="button-area">
	                    <button type="button" class="btn search" onclick="goRegist();">등록</button>
					...
					<form id="registForm" name="registForm">
	                    ...
	                    <div class="search-typeA">
	                            ...
	                            <div class="itemList">
	                                <select class="form-select" id="smstrChrtId" name="smstrChrtId" title="년도학기" style="width:300px;"><!-- 전체는 없는 것으로 함 20260609 -->
	                                    <c:forEach var="item" items="${yrSmstrList}">
									        <option value="${item.smstrChrtId}"
									            ${item.smstrChrtId eq param.smstrChrtId ? 'selected' : ''}>
									            ${item.smstrChrtnm}
									        </option>
									    </c:forEach>
	                                </select>
	                        ...
	                        <div class="item">
	                            ...
	                            <div class="itemList">    
	                            	<input type="text" id="acadSchdlExpln" name="acadSchdlExpln" value="설명">                            
	                            </div>
	                        </div> 
	                    </div>
                    </form>
