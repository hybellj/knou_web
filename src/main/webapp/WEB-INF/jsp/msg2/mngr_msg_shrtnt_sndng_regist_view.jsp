...
<script type="text/javascript">
    ...
    function fn_loadReplyInfo() {
        ajaxCall('/msgShrtntRcvnSelectAjax.do', { encParams: EPARAM, addParams: UiComm.makeEncParams({ msgShrtntSndngId: replyMsgShrtntSndngId }) }, function(res) {
    ...
    function fn_loadDetail() {
        ajaxCall('/msgShrtntSndngSelectAjax.do', { encParams: EPARAM, addParams: UiComm.makeEncParams({ msgId: msgId }) }, function(res) {
            if (res.encParams) EPARAM = res.encParams;
            if (res.result > 0 && res.returnVO) {
                let v = res.returnVO;
                $('#ttl').val(v.ttl || '');
    ...
    function fn_openRcvrPopup() {
        fn_openModal('rcvrModal', '/msgShrtntRcvrPopView.do');
    }
    ...
    function fn_openTmpltSavePopup() {
        fn_openModal('tmpltSaveModal', '/msgShrtntTmpltRegistPopView.do?ttl=' + encodeURIComponent($('#ttl').val() || '') + '&txtCts=' + encodeURIComponent($('#txtCts').val() || ''));
    }
    ...
    /* 목록으로 */
    function fn_list() {
        location.href = '/mngrMsgShrtntListView.do?encParams=' + LIST_EPARAM;
    }
</script>
