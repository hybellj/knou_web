...
<head>
    ...
    <script type="text/javascript">
         ...
         * 퀴즈 응시 관련 변수
         */
        var QUIZ_BSC_ID      = curExamBscId;
        var QUIZ_EXAM_DTL_ID = stdntExamDtlId;
        var QUIZ_TKEXAM_CNT  = ${fn:length(tkexamHstry)};
        var quizDialog       = null;
            ...
            if (tkexamHstryList.length > 0) {
                tkexamHstryList.forEach(function(v, i) {
                    ...
                    if ((v.tkexamSdttm != '') && (v.tkexamEdttm != '')) {
                        tkexamDrtn = Math.floor(v.tkexamDrtn / 60)
                                    + "<spring:message code='exam.label.min.time' /> "  /* 분 */
                                    + (v.tkexamDrtn % 60)
                                    + "<spring:message code='exam.label.sec' />";   /* 초 */
                    } else {
                        tkexamDrtn = "-";
                    }
                    html += "<tr>";
         ...
         * 3. exampprViewMv:     평가시험지 팝업 (case3)
         * 4. closeDialog:       팝업 닫기
         * 5. msgPop:            팝업 내부에서 부모창 오류 메시지 표시
         *****************************************************************************/
        /* 1 */
        function quizTkexamConfirm() {
            if (QUIZ_TKEXAM_CNT > 0) {
                ...
                var data = "encParams=" + EPARAM + "&examBscId=" + QUIZ_BSC_ID + "&examDtlId=" + QUIZ_EXAM_DTL_ID;
        ...
        function quizTkexamPopup(examBscId, examDtlId) {
            var data = "encParams=" + EPARAM + "&examBscId=" + examBscId + "&examDtlId=" + examDtlId;
