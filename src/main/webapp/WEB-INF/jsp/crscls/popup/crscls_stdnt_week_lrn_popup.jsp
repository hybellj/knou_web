...
<script type="text/javascript">
    ...
    function loadStdntInfo() {
        $.ajax({
            url: CTX + "/crscls/selectCrsStdntInfo.do",
    ...
    function loadWkDetail() {
        ...
        $.ajax({
            url: CTX + "/crscls/selectCrsStdntWkLrnSummary.do",
            type: "GET", dataType: "json",
    ...
    function renderWkSummary(d, wkNo) {
        ...
        //            stsTxt/stscrscls/stsLbl 은 화면 표시용으로 별도 분리
        var sts    = d.atndSts || "ABSNT";
        var stscrscls = sts === "ATND" ? "state_ok"   : sts === "LATE" ? "state_late" : "state_no";
        var stsLbl = sts === "ATND" ? "출석"        : sts === "LATE" ? "지각"       : "결석";
        var stsTxt = sts === "ATND" ? "○"           : sts === "LATE" ? "△"          : "X";

        $("#atndStsSpan").attr("class", stscrscls).attr("aria-label", stsLbl).text(stsTxt);
    ...
    function renderChsiList(list) {
        ...
        list.forEach(function (chsi, idx) {
            ...
            var $lbox = $(
                ...
                + '</div>'
                + '<div class="btn_right">'
                + '<label class="state ' + stscrscls + '">' + escHtml(lrnSts || '-') + '</label>'
                + '</div>'
    ...
    function loadChsiLog(cntntsId, idx) {
        ...
        $.ajax({
            url: CTX + "/crscls/selectCrsStdntLrnLog.do",
            type: "GET",
            dataType: "json",
            data: {
                sbjctId: sbjctId,
    ...
    function doAtndProcess() {
        ...
        UiComm.showMessage(wkNo + "주차 출석 처리하시겠습니까?", "confirm")
            .then(function (result) {
                if (!result) return;
                $.ajax({
                    url: CTX + "/crscls/updateCrsAtndlcProcess.do",
    ...
    function doAtndCancel() {
        ...
        UiComm.showMessage(wkNo + "주차 출석 처리를 취소하시겠습니까?", "confirm")
            .then(function (result) {
                if (!result) return;
                $.ajax({
                    url: CTX + "/crscls/updateCrsAtndlcCancel.do",
                    type: "POST", dataType: "json",
                    data: {
                        sbjctId:         sbjctId,
                        userId:          userId,
