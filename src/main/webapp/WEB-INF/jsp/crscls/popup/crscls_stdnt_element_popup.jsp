...
<script type="text/javascript">
    ...
    var elemTypeMap = {
        QNA:  { nm: "Q&A", crscls: "s_basic"  },
        ASMT: { nm: "과제", crscls: "s_work"   },
        QUIZ: { nm: "퀴즈", crscls: "s_test"   },
        SRVY: { nm: "설문", crscls: "s_basic"  },
        DSCC: { nm: "토론", crscls: "s_debate" }
    };

    $(function () {
        var info = elemTypeMap[elemType] || { nm: "학습요소", crscls: "s_basic" };
    ...
    function loadElemSbmsnList() {
        $.ajax({
            url: CTX + "/crscls/selectCrsStdntElemSbmsnList.do",
            type: "GET", dataType: "json",
    ...
    function renderElemList(list) {
        ...
        list.forEach(function (elem, idx) {
            ...
            var $lbox = $(
                ...
                + '<p class="labels"><label class="label ' + info.crscls + '">' + escHtml(info.nm) + '</label></p>'
    ...
    function loadSbmsnLog(cntntsId, idx, colCount) {
        $.ajax({
            url: CTX + "/crscls/selectCrsStdntElemSbmsnLog.do",
            type: "GET", dataType: "json",
