...
%>
<div style="font-size:12px; padding:10px; background:#f5f5f5; border:2px solid #333; margin:10px; line-height:1.6; display:none;">
    <div>[ADMIN MENU DEBUG]
    	<strong>current menuId=<%= menuId %>,
    		topMenu=<%= topMenu != null ? topMenu.getMenunm() : "NULL" %>,
    		topMenuId=<%= topMenu != null ? topMenu.getMenuId() : "NULL" %>
    	</strong>
    </div>
    <%
    if(menuId != null && !"".equals(menuId)) {
        ...
        if(currentMenu != null) {
    %>
    <div>[CURRENT MENU INFO] <strong>currentMenuNm=<%= currentMenu.getMenunm() %>, currentMenuId=<%= currentMenu.getMenuId() %>,
    upMenuIds=<%= currentMenu.getUpMenuIds() %>, menuUrl=<%= currentMenu.getMenuUrl() %>, myTopMenuId=<%= currentMenu.getMyTopMenuId()%></strong></div>
    <%
        }
    }
    %>
    ...
    if(topMenuList != null) {
        for(MenuVO top : topMenuList) {
    %>
    <div style="margin-bottom:15px; padding:10px; border:1px solid #999; background:#fff; display:none" >
			<hr style="margin:10px 0;">
			<h3>TOP MENU LIST</h3>
            <div style="margin-bottom:5px;">
                <strong style="color:blue;">[TOP]</strong>
                <%= top.getMenunm() %>
                / menuId =
                <%= top.getMenuId() %>
                <%
                if(topMenu != null
                        && top.getMenuId().equals(topMenu.getMenuId())) {
                %>
                ...
                %>
                    <li style="margin-bottom:10px;">
                        <div>
                            ...
                            / url =
                            <%= sub1.getMenuUrl() %>
                            / upMenuId = <%= sub1.getUpMenuId() %> / myTopMenuId = <%= sub1.getMyTopMenuId() %>
                        </div>
                        <%
                        ...
                        %>
                            ...
                            %>
                                <li style="margin-bottom:5px;">
                                    ...
                                    <%= sub2.getMenuId() %>
                                    / url =
                                    <%= sub2.getMenuUrl() %> / upMenuId = <%= sub2.getUpMenuId() %> / myTopMenuId = <%= sub1.getMyTopMenuId() %>
									<br>

                                    <a href="<%= sub2.getMenuUrl() %>?menuId=<%= sub2.getMyTopMenuId() %>"  arget="_blank" style="color:red;">TEST MOVE</a>
                                </li>
                            <%
                            }
                            %>
