...
<html lang="ko" style="position: fixed; width: 100%;">
	...
	<script type="text/javascript">
		var excelExampleGrid = {
			colModel:[
		        ...
		        {label:"<spring:message code='resh.label.point' />",	name:'atndEvlScr',	align:'right',	width:'5000',	colums:'D'} /* 점수 */
			]
		};

		// 세미나성적엑셀업로드
		function smnrScrExcelUpload() {
			let dx = dx5.get("fileUploader");
			const url  = "/smnr/profSmnrScrExcelUpload.do";
			const data = {
				smnrId   	: "${vo.smnrId}",
			...
			ajaxCall(url, data, function(data) {
				if (data.result > 0) {
					window.parent.smnrAtndListSelect();
		...
		// 엑셀샘플다운로드
		function excelSampleDown() {
	        $("#excelGrid").val(JSON.stringify(excelExampleGrid));
			$("#smnrScrRegistForm").attr("target", "exampleExcelDownloadIfm");
	        $("#smnrScrRegistForm").attr("action", "/smnr/profSmnrScrRegistSampleExcelDown.do");
	        $("#smnrScrRegistForm").submit();
		}
	...
	<form id="smnrScrRegistForm" name="smnrScrRegistForm" method="POST">
	...
	<body class="modal-body">
        ...
        <div class="board_top">
	    	<button class="btn basic" onclick="excelSampleDown()"><spring:message code="exam.button.excel.sample.down" /></button><!-- 엑셀 샘플 다운로드 -->
        </div>
