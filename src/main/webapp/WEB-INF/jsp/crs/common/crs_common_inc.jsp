...
<script type="text/javascript">
    ...
    var CrsMessage = window.CrsMessage || {};
    // 서버 응답 메시지가 없을 때 사용할 기본 메시지를 반환한다.
    CrsMessage.result = function(data, fallbackMessage) {
        return data && data.message ? data.message : fallbackMessage;
    };
    // CRS 화면에서 공통 메시지 UI를 표시한다.
    CrsMessage.show = function(message, type) {
        if (message && window.UiComm && UiComm.showMessage) {
            UiComm.showMessage(message, type || "warning");
        }
    };
    window.CrsMessage = CrsMessage;
    window.crsResultMessage = CrsMessage.result;

    /**
     * 루브릭 화면 이동
     * @param {String} tab      - "list"|"write"
     * @param {String} rubricId - 루브릭 ID (write 시 필요, 신규 등록은 '')
     */
    function rubricViewMv(tab, rubricId) {
        var urlMap = {
            "list"  : "/crs/defaultRubricView.do",
            "write" : "/crs/rubricWriteView.do"
        };
        if (tab === "list") {
            document.location.href = urlMap[tab] + "?encParams=" + EPARAM;
        } else {
            var extData = { rubricId: rubricId || '' };
