...
<head>
	...
	<script type="text/javascript">
		 ...
		 * @param pageNo	이동페이지
		 */
		function srvyRsltListSelect(pageNo) {
			PAGE_INDEX   = pageNo || PAGE_INDEX;
			LIST_SCALE	 = LIST_SCALE || 20;
			SEARCH_VALUE = $("#searchValue").val();

			const url 	= "/srvy/admSrvyRsltListAjax.do";
			const param = {
		...
		function createListHTML(list) {
			...
			list.forEach(function(v,i) {
				dataList.push({
					...
					ptcpSdttm: 		UiComm.formatDate(v.ptcpSdttm, "datetime2")
				});
			});
		...
		function srvyRspnsExcelDown() {
			...
			submitForm("/srvy/admRspnsStatusExcelDown.do", kvArr);
		}
		...
		function srvyPtcpStatusExcelDown() {
			...
			submitForm("/srvy/admPtcpStatusExcelDown.do", kvArr);
		}

		// 전체설문현황
		function srvyStatus() {
			const url  = "/srvy/admSrvyPtcpStatusAjax.do";
			const data = {
				orgId 			: $("#orgId").val(),
				smstrChrtId 	: $("#smstrChrtId").val(),
				srvyTrgtTycd	: $("#srvyTrgtTycd").val(),
			...
			$.ajax({
                ...
                success		: function (data) {
                    if (data.result > 0) {
                    	...
                    	let html = htmlOption.createAdmSrvyPtcpStatusHTML(returnVO);	// 관리자설문참여현황 HTML 리턴

                    	$("#srvyStatusDiv").empty().html(html);
                    	srvyCommon.statusChartSet('status', returnVO.cmmnCdList.cntnDvcTycd, returnVO.egovListMap.ptcpDvcList, returnVO.egovMap);	// 결과차트출력
                		srvyCommon.statusChartSet('device', returnVO.cmmnCdList.cntnDvcTycd, returnVO.egovListMap.ptcpDvcList, returnVO.egovMap);	// 결과차트출력
            ...
            <div id="content" class="content-wrap common">
                <div class="admin_sub">
                    <div class="sub-content">
                        ...
                        <div class="board_top">
							<h3 class="board-title"><spring:message code="srvy.label.all.srvy.result" /><!-- 전체설문 결과 --></h3>
							<div class="right-area">
								<button type="button" class="btn type2 big" onclick="srvyViewMv('', 'ADMRSLTLIST')"><spring:message code="srvy.button.list" /><!-- 목록 --></button>
							</div>
						</div>
						...
						<div class="board_top">
							<h3><spring:message code="srvy.label.target" /><!-- 대상자 --></h3>
							<div class="right-area">
								<button type="button" class="btn type2" onclick="srvyRspnsExcelDown()"><spring:message code="srvy.button.excel.down.all.srvyppr" /><!-- 전체설문지 엑셀다운로드 --></button>
								<button type="button" class="btn type2" onclick="srvyPtcpStatusExcelDown()"><spring:message code="srvy.button.excel.down.all.srvy.result" /><!-- 전체설문 결과 엑셀다운로드 --></button>
							</div>
						</div>
						...
						<div class="board_top">
                            ...
                            <select class="form-select wide" id="srvyTrgtTycd" onchange="srvyRsltListSelect(1)">
                            ...
                            <select class="form-select wide" id="srvyPtcp" onchange="srvyRsltListSelect(1)">
                            	<option value=""><spring:message code="srvy.common.all" /><!-- 전체 --></option>
                            ...
                            <button type="button" class="btn basic icon search" aria-label="검색" onclick="srvyRsltListSelect(1)"><i class="icon-svg-search"></i></button>
							...
							<script>
								...
								let listTable = UiTable("list", {
									...
									columns: [
										...
										{title:"<spring:message code='srvy.label.dept' />", 		field:"deptnm", 		headerHozAlign:"center", 	hozAlign:"center",	width:180,		minWidth:180},/* 학과 */
										{title:"<spring:message code='srvy.label.user.type' />", 	field:"authrtnm", 		headerHozAlign:"center", 	hozAlign:"center",	width:180,		minWidth:180},/* 사용자 구분 */
										{title:"<spring:message code='srvy.label.user.id' />", 		field:"userId", 		headerHozAlign:"center", 	hozAlign:"center",	width:180,		minWidth:180},/* 학번/사번 */
										{title:"<spring:message code='srvy.label.user.nm' />", 		field:"usernm",		 	headerHozAlign:"center", 	hozAlign:"center",	width:0,		minWidth:150},/* 이름 */
