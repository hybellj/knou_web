    ...
    LiveTabs.prototype.createTab = function ( TabTitle , tab_id ,  fallback = '' ){
      ...
      // console.log(document.getElementById('LiveTabsContainer').innerHTML);
       ...
       $(config.tabContainer).innerHTML+="<li id=\""+this.div+""+id+"\" class=\"d-inlineblock nav-item\" ><a href=\"javascript:void(0);return false;\" id=\""+this.div+"a"+id+"\" onclick=\""+this.objname+".switchTab(\'"+id+"\');return false\" title=\""+TabTitle+"\" class=\"nav-link\"><span>"+TabTitle+"</span><button type=\"button\" class=\"btn-close ms-3 liveTab-closebtn\" aria-label=\"Close\" onclick=\""+this.objname+".closeTab('"+id+"')\"></button></a></li>\n";
		...
		$(this.config.tabContentContainer).innerHTML+=
		//"<iframe id=\"" + this.config.tabContentContainer + id + "\" onload='javascript:resizeIframe(this);' style=\"width:100%; min-height: 100%; border:solid; color:red;\"></iframe>"
		"<iframe id=\"" + this.config.tabContentContainer + id + "\" style=\"width:100%; min-height: 100%; border:solid; color:red;\"></iframe>"
		;
		
		// iframe 참조
		const iframe = document.getElementById(this.config.tabContentContainer + id);
		
		// 로드 이벤트 등록
		iframe.onload = function() {
		    // offsetHeight + (scrollHeight - clientHeight) 방식
		    iframe.style.height = iframe.offsetHeight + (iframe.contentDocument.body.scrollHeight - iframe.clientHeight)+ 1 + "px";
       }
 ...
 * resizeFrameById
 * 
 * @description : 주어진 iframe의 콘텐츠 높이에 맞춰 iframe 높이를 자동으로 조절
 * @param {string} tabId - 높이를 조절할 iframe의 id
 * @usage  : iframe 내부 AJAX 완료 후 window.parent.resizeFrameById(myId) 호출
 */
function resizeFrameById(frameId) {
	const iframe = document.getElementById(frameId);
	
	// 프레임이 존재하는 경우에만 작동하도록
	if(iframe) {
		iframe.style.height = iframe.offsetHeight + (iframe.contentDocument.body.scrollHeight - iframe.clientHeight) + "px";
	}
}