...
function createForumCommentListHTML(commentList) {
    ...
    commentList.forEach(function(rs) {
            html += "      <ul" + (rs.level === 2 ? " class='co_inner re_comment_ul'" : "") + ">";
            html += "        <li" + (rs.level === 2 ? " class='re_comment'" : "") + ">";
            html += "          <div class='item'>";
            html += "            <div class='cmt_info'>";
            html += "            <span class='flex-item'><em class='mra mt0'>" + escapeHtml(rs.regNm) + " <code>" + formatDttm(rs.modDttm) + " | 글자수: " + rs.cmntCtsLen + "자</code></em></span>";
            html += "            </div>";
