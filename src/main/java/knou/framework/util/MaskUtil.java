...
public class MaskUtil {
		...
		if (userNm != null && !"".equals(userNm)) {
	        ...
	        if (isKorean) { // 한글이름
	        	...
	        	if (length <= 2) {
	                ...
	                return userNm.charAt(0) + "*" + userNm.charAt(2);
	            } else if (length == 4) {
	                return userNm.charAt(0) + "**" + userNm.charAt(3);
	            } else {
    ...
    public static String maskJuminNo(String juminNo) {
    	if (juminNo != null && !"".equals(juminNo)) {
    		if (juminNo.contains("-")) {
    			return juminNo.replaceAll("(?<=\\d{6}-)\\d{7}", "*******"); // 901231-1234567 (하이픈이 존재하는 경우)
			} else {
				return juminNo.replaceAll("\\d{7}$", "*******"); // 9012311234567 (하이픈이 존재하지 않는 경우)
			}
    	}
		else {
			return juminNo;
