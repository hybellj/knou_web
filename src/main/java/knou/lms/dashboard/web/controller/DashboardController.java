...
public class DashboardController extends ControllerBase {
	...
	public String main(HttpServletRequest request, HttpServletResponse response, ModelMap model,
		...
		if (CommConst.AUTHRT_GRPCD_ADM.equals(menuType)) {
			// 관리자
			returnUri = "/adminDashboardV2.do?menuId=ADMORG0000001";
		} else if (CommConst.AUTHRT_GRPCD_PROF.equals(menuType)) {
			// 교수
			...
			if (!SessionInfo.isVirtualLogin(request) && reloginChk && "Y".equals(chk) && "0".equals(loginGbn)) {
	...
	public String main(HttpServletRequest request, HttpServletResponse response, DashboardAdminVO vo, ModelMap model)
			throws Exception {
		...
		// 현재 홈정보 저장
		SessionInfo.setCurUserHome(request, "/dashboard/adminDashboardV2.do?menuId=ADMORG0000001");
		...
		if ("".equals(lastLogin)) {
			...
			if (loginUser != null) {
				SessionInfo.setLastLogin(request, DateTimeUtil.getDateType(8, loginUser.get("lgnDttm") + ".") + " ("
						+ loginUser.get("lgnIp") + ")");
			}
		}
		...
		model.addAttribute("menuType", CommConst.AUTHRT_GRPCD_ADM);
	 ...
	 * @throws Exception *********************************
	 */
	@RequestMapping(value = "/adminDashboardV2.do")
	public String adminDashboardV2(DashboardAdminVO vo, @CurrentUser UserContext userCtx, Model model) {

		// 조회필터옵션 세팅
		EgovMap filterOptions = dashboardFacadeService.loadFilterOptions(userCtx);
		model.addAttribute("filterOptions", filterOptions);

		DashboardViewModel dashVM = dashboardFacadeService.admDashboardViewModel(5);

		model.addAttribute("menuId", "ADMORG0000001");
		model.addAttribute("sysNoticeList", dashVM.getAdmDashSysNoticeList());
		model.addAttribute("allNoticeList", dashVM.getAdmDashAllNoticeList());
		model.addAttribute("allSrvyList", dashVM.getAdmDashAllSrvyList());
		model.addAttribute("encParams", getEncParams());

		return "/common_new/admin_dashboard";
	}
	...
	public String adminDashboard(HttpServletRequest request, HttpServletResponse response, DashboardAdminVO vo,
		...
		if ("".equals(lastLogin)) {
			...
			if (loginUser != null) {
				SessionInfo.setLastLogin(request, DateTimeUtil.getDateType(8, loginUser.get("loginTryDttm") + ".")
						+ " (" + loginUser.get("connIp") + ")");
			}
		}
		...
		// 관리자메뉴
		model.addAttribute("orgId", SessionInfo.getOrgId(request));
		model.addAttribute("authrtCd", CommConst.AUTHRT_GRPCD_ADM);
	...
	public String profDashboard(DashboardVO dashboardVO, @CurrentUser UserContext userCtx, HttpServletRequest request,
		...
		// 기관/사용자유형 가져오기
		String headerOrgId = dashboardVO.getHeaderOrgId();
		String headerUserTycd = dashboardVO.getHeaderUserTycd();
		// 학기 가져오기
		String headerSmstrChrtId = dashboardVO.getHeaderSmstrChrtId();

		// 과목 기관목록
		List<SubjectOrgDTO> subjectOrgList = userCtx.getSubjectOrgList();

		// 기관/사용자유형을 전달받지 않을 경우 기본값 설정
		if (headerOrgId == null || headerOrgId.isEmpty()) {
			if (subjectOrgList != null && !subjectOrgList.isEmpty()) {
				SubjectOrgDTO subjectOrgDTO = subjectOrgList.getFirst();
				headerOrgId = subjectOrgDTO.getOrgId();

				if ("ALL".equals(subjectOrgDTO.getOrgnm())) {
					headerUserTycd = "";
				}
				else {
					List<String> tycdList = subjectOrgDTO.getOrgUserTycdList();
					if (tycdList != null && tycdList.size() == 1) {
						headerUserTycd = (tycdList == null || tycdList.isEmpty()) ? "" : tycdList.getFirst();
					}
					else {
						headerUserTycd = "";
					}
			...
			dashboardVO.setHeaderOrgId(headerOrgId);
			dashboardVO.setHeaderUserTycd(headerUserTycd);
		}

		// 사용자의 기본 학기 정보 가져오기
		if (headerSmstrChrtId == null || headerSmstrChrtId.isEmpty()) {
			List<SmstrChrtVO> userSmstrList = userCtx.getSmstrChrtList(headerOrgId, headerUserTycd);

			if (userSmstrList != null && !userSmstrList.isEmpty()) {
				headerSmstrChrtId = userSmstrList.get(0).getSmstrChrtId();
				dashboardVO.setHeaderSmstrChrtId(headerSmstrChrtId);
			}
		}

	    model.addAttribute("dashboardVO", dashboardVO);
		model.addAttribute("orgId", headerOrgId);
		model.addAttribute("userTycd", headerUserTycd);
		model.addAttribute("smstrChrtId", headerSmstrChrtId);
		model.addAttribute("authrtGrpcd", userCtx.getAuthrtGrpcd());
		model.addAttribute("userId", userCtx.getUserId());

		// 암호화파라메터에 설정
		resetEncParam();
		addEncParam("orgId", headerOrgId);
		addEncParam("userTycd", headerUserTycd);
		addEncParam("smstrChrtId", headerSmstrChrtId);

		return "dashboard/prof_dashboard";
	}
	...
	public String schCalendar(SchVO vo, ModelMap map, HttpServletRequest request) throws Exception {
		...
		vo.setUserId(userId);
		if (!"".equals(menuType)) {
			String[] userTypes = "|".equals(menuType.substring(0, 1)) ? menuType.substring(1).split("\\|")
					: menuType.split("\\|");
	...
	@ResponseBody
	public ProcessResultVO<SchVO> schCalendarList(SchVO vo, ModelMap map, HttpServletRequest request,
		...
		try {
			...
			/*
			 * String creCrsListString = request.getParameter("coList"); creCrsListString =
			 * creCrsListString.replace("[", "").replace("]", ""); String[] creCrsListArray
			 * = creCrsListString.split(","); List<String> creCrsList =
			 * Arrays.asList(creCrsListArray); vo.setCreCrsList(creCrsList);
			 */
	...
	@ResponseBody
	public ProcessResultVO<SchVO> stuSchCalendarList(SchVO vo, ModelMap map, HttpServletRequest request)
			throws Exception {
	...
	@ResponseBody
	public ProcessResultVO<SchVO> profSchCalendarList(SchVO vo, ModelMap map, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
	...
	@ResponseBody
	public ProcessResultVO<SchVO> acadSchList(SchVO vo, ModelMap map, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
	...
	@ResponseBody
	public ProcessResultVO<CreCrsVO> listUserCreCrs(CreCrsVO vo, ModelMap map, HttpServletRequest request)
			throws Exception {
		String userId = StringUtil.nvl(SessionInfo.getUserId(request));
	...
	@ResponseBody
	public ProcessResultVO<AcadSchVO> writeSch(AcadSchVO vo, ModelMap map, HttpServletRequest request)
			throws Exception {
	 ...
	 * @throws Exception
	 ******************************************************/
	@RequestMapping(value = "/admLrnStsBySbjctListAjax.do")
	@ResponseBody
	public ProcessResultVO<EgovMap> lrnStsBySbjctListAjax(DefaultVO vo) {
		return new ProcessResultVO<EgovMap>().setReturnList(dashboardService.lrnStsListBySbjct(vo)).setResultSuccess();
	}
	...
	public String downExcelLessonStatusByCrs(DashboardVO vo, ModelMap model, HttpServletRequest request)
			throws Exception {
		String orgId = SessionInfo.getOrgId(request);
		...
		// 조회조건
		String[] searchValues = { getMessage("common.year") + " : " + vo.getHaksaYear() // 학년도
	 ...
	 * @throws Exception
	 ******************************************************/
	@RequestMapping(value = "/admLrnStsByStdListAjax.do")
	@ResponseBody
	public ProcessResultVO<EgovMap> listLessonStatusByStd(DashboardVO vo) {
		ProcessResultVO<EgovMap> resultVO = new ProcessResultVO<>();

		List<EgovMap> list = dashboardService.lrnStsListByStd(vo);
	...
	public String downExcelLessonStatusByStd(DashboardVO vo, ModelMap model, HttpServletRequest request)
			throws Exception {
		String orgId = SessionInfo.getOrgId(request);
	...
	public String lessonStatusPop(DashboardVO vo, ModelMap model, HttpServletRequest request) throws Exception {
		...
		List<EgovMap> listLessonStatusByStd = dashboardService.lrnStsListByStd(vo);
	 ...
	 * @throws Exception
	 ******************************************************/
	@RequestMapping(value = "/admUserListAjax.do")
	@ResponseBody
	public ResultDTO admUserListAjax(DashboardVO vo, @CurrentUser UserContext userCtx) {

		ResultDTO resusltDTO = new ResultDTO<>(dashboardService.userList(vo));
		resusltDTO.setResultSuccess();
	...
	public String lectEvalPop(DashboardVO vo, ModelMap model, HttpServletRequest request) throws Exception {
		...
		model.addAttribute("lectEvalPopUrl",
				ErpUtil.getStuLectEvalUrl(termVO.getHaksaYear(), termVO.getHaksaTerm(), userId));
	...
	@ResponseBody
	public String closeVirtualSession(ModelMap map, HttpServletRequest request, HttpServletResponse response)
			throws Exception {
	...
	public String closeVirtualSessionByApp(ModelMap map, HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		resetVirtualSession(request);

		String url = StringUtil.nvl(request.getParameter("url"));
		if ("".equals(url)) {
			url = "/dashboard/adminDashboardV2.do";
		}
	...
	public String widgetSettingPop(HttpServletRequest request, HttpServletResponse response, ModelMap model)
			throws Exception {
		String userId = SessionInfo.getUserId(request);
	...
	public String widgetSettingPop2(HttpServletRequest request, HttpServletResponse response, ModelMap model)
			throws Exception {
		String userId = SessionInfo.getUserId(request);
	...
	@ResponseBody
	public ProcessResultVO<DashboardVO> widgetSettingPopList(DashboardVO vo, ModelMap map, HttpServletRequest request)
			throws Exception {
		String userId = StringUtil.nvl(SessionInfo.getUserId(request));
		...
		if (widgetInfo != null) {
			...
			if (jsonCts != null && !jsonCts.isEmpty()) {
				...
				try {
					...
					/*
					 * // 과목 접속자 수 조회 EgovMap lgnUsrCnt = dashboardService.lgnUsrCntSelect(vo);
					 * result.put("lgnUsrCnt", lgnUsrCnt);
					 *
					 * // 학생 수 조회 EgovMap totStdntCnt = dashboardService.totStdntCntSelect(vo);
					 * result.put("totStdntCnt", totStdntCnt);
					 *
					 * // 과목 리스트 조희 List<EgovMap> sbjctList = dashboardService.sbjctList(vo);
	...
	@ResponseBody
	public ProcessResultVO<DashboardVO> widgetStngChange(DashboardVO vo, ModelMap map, HttpServletRequest request)
			throws Exception {
	...
	@ResponseBody
	public ProcessResultVO<DashboardVO> widgetStngDelete(DashboardVO vo, ModelMap map, HttpServletRequest request)
			throws Exception {
		ProcessResultVO<DashboardVO> resultVO = new ProcessResultVO<DashboardVO>();
	...
	public String dashboard(@CurrentUser UserContext userCtx, HttpServletRequest request, ModelMap model)
			throws Exception {

		DashboardViewModel dashVM = dashboardFacadeService.getDashboardResponse(userCtx);
	...
	public String profWidgetQna(DashboardVO vo, @CurrentUser UserContext userCtx, HttpServletRequest request, ModelMap model) throws Exception {
		...
		//	게시판미열람수조회
		subjectVM.setBadge(bbs2Service.profBbsUnreadCntSelect(sbjctDto));
	...
	public String profWidgetNotice(DashboardVO vo, @CurrentUser UserContext userCtx, HttpServletRequest request, ModelMap model) throws Exception {

		SubjectDTO subjectDto = new SubjectDTO(userCtx);

		DashboardViewModel dashVM = new DashboardViewModel();

		//	교수대시보드 전체+과목 공지목록조회
		dashVM.setProfDashAllNoticeList(bbs2Service.profDashAllNoticeList(subjectDto));

		//	교수대시보드 과목공지목록조회
		dashVM.setProfDashSubjectNoticeList(bbs2Service.profDashSubjectNoticeList(subjectDto));

		//	최신과정공지목록조회(전체)
		dashVM.setDashCrsNoticeList(bbs2Service.dashCrsNoticeList(subjectDto));
	...
	public String profWidgetCounsel(DashboardVO vo, @CurrentUser UserContext userCtx, HttpServletRequest request, ModelMap model) throws Exception {
	...
	public String profWidgetSubject(DashboardVO dashboardVO, @CurrentUser UserContext userCtx, HttpServletRequest request,
			ModelMap model) throws Exception {

		DashboardViewModel dashVM = new DashboardViewModel();
		CommonDTO cmmnDto = new CommonDTO(userCtx);

		// 전달받은 기관,학기 정보 셋팅
		cmmnDto.setOrgId(dashboardVO.getOrgId());
		cmmnDto.setSmstrChrtId(dashboardVO.getSmstrChrtId());

		// 교수대시보드 강의과목목록조회
		dashVM.setLctrSbjctSummaryList(subjectService.profSubjectSummaryList(cmmnDto));
		model.addAttribute("dashVM", dashVM);
	...
	/**
	 * *************************************************** 학생 대시보드 TODO: TOBE 새로
	 * 작업하는 학생용 대시보드 작업 작업 완료후 새 버전에 맞게 명칭, 메쏘드 등 변경
	 */
	@RequestMapping(value = "/stuDashboard.do")
	public String stuDashboard(DashboardVO dashboardVO,
            @CurrentUser UserContext userCtx, HttpServletRequest request,
			HttpServletResponse response, ModelMap model) throws Exception {
		...
		if (headerOrgId == null || headerOrgId.isEmpty()) {
			...
			if (subjectOrgList != null && !subjectOrgList.isEmpty()) {
				...
				if ("".equals(subjectOrgDTO.getOrgId())) {
					headerUserTycd = "STDNT";
				}
				else {
					List<String> tycdList = subjectOrgDTO.getOrgUserTycdList();
					headerUserTycd = (tycdList == null || tycdList.isEmpty()) ? "STDNT" : tycdList.getFirst();
				}
			}
	...
	public String stuWidgetQna(DashboardVO vo, @CurrentUser UserContext userCtx, HttpServletRequest request, ModelMap model) throws Exception {
	...
	public String stuWidgetNotice(DashboardVO vo, @CurrentUser UserContext userCtx, HttpServletRequest request, ModelMap model) throws Exception {
		...
		//	학생대시보드 전체 목록조회
		dashVM.setStdntDashAllNoticeList(bbs2Service.stdntDashAllNoticeList(subjectDto));

		//	학생기관공지(전체공지)
		dashVM.setDashCrsNoticeList(bbs2Service.dashCrsNoticeList(subjectDto));

		//	학생과목공지
		dashVM.setStdntDashSubjectNoticeList(bbs2Service.stdntDashSubjectNoticeList(subjectDto));
	...
	public String stuWidgetSubject(DashboardVO dashboardVO, @CurrentUser UserContext userCtx,
		...
		// 학생대시보드 강의과목목록조회
		dashVM.setLctrSbjctSummaryList(subjectService.stdntSubjectSummaryList(cmmnDto));
		model.addAttribute("dashVM", dashVM);
	...
	public String stuWidgetPds(DashboardVO vo, @CurrentUser UserContext userCtx, HttpServletRequest request, ModelMap model) throws Exception {
