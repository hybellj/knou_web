...
public class SysMgrErrVO extends DefaultVO {
	...
	public SysMgrErrVO() {} // 기본생성자필요 // 아래 파라미터 생성자를 만들 경우 기본생성자를 명시적으로 선언해야함

	public SysMgrErrVO(UserContext userCtx) {
		this.setOrgId(userCtx.getLoginUser().getOrgId());
		this.setDeptId(userCtx.getLoginUser().getDeptId());
		this.setStdntNo(userCtx.getLoginUser().getStdntNo());
		this.setUserNm(userCtx.getLoginUser().getUsernm()); 
	}
