CREATE OR REPLACE PACKAGE CRYPTO IS
    ...
    /* Final Function */
    FUNCTION PASSWORD(x IN VARCHAR2) RETURN VARCHAR2;
    FUNCTION PASSWORD_RAW(x IN RAW) RETURN VARCHAR2;
    FUNCTION ENCRYPT(inKey IN VARCHAR2, inValue IN VARCHAR2) RETURN VARCHAR2;
	FUNCTION DECRYPT(inKey IN VARCHAR2, inValue IN VARCHAR2) RETURN VARCHAR2;
...
CREATE OR REPLACE PACKAGE BODY CRYPTO IS
    ...
    FUNCTION PASSWORD(x IN VARCHAR2) RETURN VARCHAR2 AS
        ctx TR_CTX;
        res TA_NUMBER;
    BEGIN
        RETURN PASSWORD_RAW(utl_raw.cast_to_raw(x));
    END;

    FUNCTION PASSWORD_RAW(x IN RAW) RETURN VARCHAR2 AS
        ctx TR_CTX;
        res TA_NUMBER;
    BEGIN
        sha256_init_ctx (ctx);
   ...
   FUNCTION ENCRYPT(inKey IN VARCHAR2, inValue IN VARCHAR2) RETURN VARCHAR2 AS
      encrypted_raw RAW(2000);
      key_bytes_raw RAW(32) := UTL_RAW.CAST_TO_RAW(inKey);
      encryption_type PLS_INTEGER := DBMS_CRYPTO.ENCRYPT_AES256 + DBMS_CRYPTO.CHAIN_CBC + DBMS_CRYPTO.PAD_PKCS5;
   BEGIN
      encrypted_raw := DBMS_CRYPTO.ENCRYPT(
         SRC => UTL_I18N.STRING_TO_RAW(inValue, 'AL32UTF8'),
         TYP => encryption_type,
         KEY => key_bytes_raw
      );
   ...
   FUNCTION DECRYPT(inKey IN VARCHAR2, inValue IN VARCHAR2) RETURN VARCHAR2 AS
      decrypted_raw RAW(2000);
      key_bytes_raw RAW(32) := UTL_RAW.CAST_TO_RAW(inKey);
      decryption_type PLS_INTEGER := DBMS_CRYPTO.ENCRYPT_AES256 + DBMS_CRYPTO.CHAIN_CBC + DBMS_CRYPTO.PAD_PKCS5;
   BEGIN
      decrypted_raw := DBMS_CRYPTO.DECRYPT(
         SRC => HEXTORAW(inValue),
         TYP => decryption_type,
         KEY => key_bytes_raw
      );
