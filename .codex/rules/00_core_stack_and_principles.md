...
-   Front: JSTL + jQuery
-   통신: AJAX
-   신규/리팩터링 AJAX 응답은 `ResultDTO<T>`를 표준으로 사용한다.
-   `ProcessResultVO<T>`는 레거시 유지가 필요한 기존 기능에서만 허용한다.

## 금지/주의 (프로젝트 기본 원칙)

-   Oracle 12c 문법 금지
-   MyBatis resultType은 VO를 우선한다. 단, 단순 목록/통계/운영성 조회처럼 VO 생성 이득이 낮은 경우 `EgovMap`을 허용한다.
...
### F. Analysis Guardrail

-   원인 분석 설명은 확정된 사실과 가설을 구분해서 작성한다.
-   모든 가설에는 검증 방법 또는 반증 조건을 함께 적는다.
-   UI 이슈는 CSS를 최종 후보로 보고, 다음 순서로 먼저 확인한다.
    1. 컴포넌트 초기화 시점
    2. 데이터 바인딩 시점
    3. hidden/accordion/tab 내부 초기화 상태
    4. load 이후 부모 폭 변화 여부
    5. redraw/resize API 존재 여부
    6. 마지막에 CSS 폭 계산
-   사용자가 특정 변경으로 증상이 사라졌다고 알려주면, 그 변경을 최우선 근거로 사용해 원인을 다시 평가한다.
-   최종 원인 설명은 아래 형식을 따른다.
    -   확정된 사실
    -   유력한 원인
    -   배제된 가설
    -   미검증 사항
-   반증되지 않은 추측을 확정된 원인처럼 서술하지 않는다.
